            ...
            {refundPhase === "confirm" ? (
              <>
                <p className="mb-4 text-sm leading-relaxed text-zinc-600">
                  {(() => {
                    const provider = (
                      refundTarget.provider ||
                      refundTarget.source ||
                      ""
                    ).toLowerCase();
                    if (provider === "yookassa") {
                      return "Будет выполнен процесс возврата этого платежа через API ЮКассы. Отменить эту операцию невозможно.";
                    }
                    if (provider === "manual") {
                      return "Это ручной грант (книга/вебинар), не платёж Lava.top. Подтверждение только сменит статус на «Возврат» и снимет доступ по этому гранту. Денег в шлюзе нет — ничего оформлять снаружи не нужно.";
                    }
                    return "Отправьте заявку на возврат в админке LavaTop. Для этого в списке платежей нужно кликнуть на платеже. Здесь же можно только изменить статус данного платежа на «Возврат». В результате запись о нём сохранится, но он не будет учитываться в статистике как оплаченный.";
                  })()}
                </p>

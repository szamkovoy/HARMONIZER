...
export default function AdminUserStatsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <Link href="/admin/users" className="mb-4 inline-flex items-center gap-1.5 text-sm text-zinc-400 hover:text-zinc-800">
        <ArrowLeft size={15} /> К списку пользователей
      </Link>

      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-zinc-900">Статистика пользователей</h1>
        ...
        <div className="flex flex-wrap gap-2">
          {PERIODS.map((value) => (
            <button
              ...
              className={`rounded-xl px-3 py-2 text-sm ${days === value ? "bg-emerald-600 font-semibold text-white" : "border border-zinc-200 text-zinc-700 hover:bg-zinc-100"}`}
            >
              {value} дн.
            </button>
          ))}
          <button
            ...
            className={`rounded-xl px-3 py-2 text-sm ${grain === "day" ? "bg-emerald-600 font-semibold text-white" : "border border-zinc-200 text-zinc-700 hover:bg-zinc-100"}`}
          >
            Дни
          </button>
          <button
            ...
            className={`rounded-xl px-3 py-2 text-sm ${grain === "week" ? "bg-emerald-600 font-semibold text-white" : "border border-zinc-200 text-zinc-700 hover:bg-zinc-100"}`}
          >
            Недели
          </button>
        </div>
      ...
      {stats ? (
        <div className="flex flex-col gap-4">
          ...
          <section className="rounded-xl border border-zinc-200 bg-white p-4">
            <h2 className="mb-3 text-sm font-bold text-zinc-900">Доступ сейчас</h2>
            <div className="grid gap-2 sm:grid-cols-4">
              {Object.entries(ACCESS_LABELS).map(([key, label]) => (
                <div key={key} className="rounded-lg border border-zinc-100 bg-zinc-50 p-3">
                  ...
                  <div className="text-xl font-bold text-zinc-900">{stats.by_access[key] ?? 0}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-xl border border-zinc-200 bg-white p-4">
            <h2 className="mb-3 text-sm font-bold text-zinc-900">Тарифы в БД (сырые)</h2>
            <div className="grid gap-2 sm:grid-cols-2">
              {Object.entries(TIER_LABELS).map(([tier, label]) => (
                <div key={tier} className="rounded-lg border border-zinc-100 bg-zinc-50 p-3">
                  <div className="text-xs text-zinc-500">{label}</div>
                  <div className="text-xl font-bold text-zinc-900">{stats.by_tier[tier] ?? 0}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-xl border border-zinc-200 bg-white p-4">
            <h2 className="mb-3 text-sm font-bold text-zinc-900">Динамика регистраций</h2>
            <div className="flex max-h-96 flex-col gap-2 overflow-y-auto">
              ...
              {stats.registration_series.map((item) => (
                <div key={item.date} className="grid grid-cols-[96px_1fr_32px] items-center gap-3">
                  ...
                  <div className="text-right text-xs text-zinc-700">{item.count}</div>
                </div>
              ))}
            </div>
          </section>

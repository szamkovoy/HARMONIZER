...
export default function AdminUserStatsPage() {
  ...
  useEffect(() => {
    setStats(null);
    adminFetch<Stats>(`/api/admin/users/stats?days=${days}`)
      .then(setStats)
      .catch((err) => setError(err instanceof Error ? err.message : "Не удалось загрузить статистику"));
  }, [days]);

  const maxReg = Math.max(...(stats?.registration_series.map((x) => x.count) ?? [1]));
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500">Регистрации по дням, текущая структура тарифов и активность за последние 24 / 72 / 168 часов.</p>
        </div>
      ...
      {stats ? (
        <div className="flex flex-col gap-4">
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <Card title={`Регистрации за ${stats.period_days} дн.`} value={String(stats.registrations_in_period)} hint="Скользящее окно, не календарный месяц" />
          ...
          <section className="rounded-xl border border-white/10 bg-[rgba(30,32,38,0.92)] p-4">
            <h2 className="mb-3 text-sm font-bold text-zinc-100">Тарифы сейчас</h2>

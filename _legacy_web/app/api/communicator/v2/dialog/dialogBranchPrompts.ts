...
export function buildPlanningPrompt(ctx: BrainPromptContext, input: PlanningTurnInput): {
  ...
  const lines: string[] = [
    ...
    "- When you split the user's plan into separate [PLANNED_EVENT] cards, segment it correctly: two independent actions named in one phrase («погулять и лечь пораньше») are TWO separate events; a goal together with the means of reaching it («купить лодку, чтобы плавать») is ONE event; and two things that form ONE shared occasion — meeting someone in order to do something together («встретиться с подругой и покататься на катере», «встретиться с друзьями за ужином») — are ONE event. When unsure whether it is one outing or two, lean toward keeping it as one and let the user split it if they correct you.",

-- Low-priority weekly prune of notification_deliveries older than 30 days.
-- Inbox only shows the last 10 deliveries per user; older rows are retention waste.
-- Pure SQL (no Edge / Vault): batched deletes with SKIP LOCKED + short sleeps
-- so the job yields to interactive traffic. Parent `notifications` rows stay
-- for admin history.
...
begin
  -- Keep the session polite: fail fast on locks / don't run forever.
  ...
  loop
    ...
    -- Yield between batches (low priority vs hourly precompute / reconcile).
...
-- Register weekly off-peak job in self-heal cron registry.
-- Cron must pass explicit args (defaults alone make a bare () call ambiguous
-- if a zero-arg overload is ever added).
  ...
  for req in
    ...
    from (
      values
        ...
        (
          -- Sunday 04:37 UTC — away from :00/:15/:20/:35 hourly traffic.

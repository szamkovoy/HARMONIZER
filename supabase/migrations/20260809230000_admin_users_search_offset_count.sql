-- Admin users list: pagination (offset) + total count for filters.
...
-- Count matching users (same filters as search, no sort/limit).
...
revoke all on function public.admin_count_users(
  text, text, text, text, text, text,
  timestamptz, timestamptz, timestamptz, timestamptz,
  text, int, int, text
) from public, anon, authenticated;
grant execute on function public.admin_count_users(
  text, text, text, text, text, text,
  timestamptz, timestamptz, timestamptz, timestamptz,
  text, int, int, text
) to service_role;

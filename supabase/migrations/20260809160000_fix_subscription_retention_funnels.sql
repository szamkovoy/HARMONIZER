-- Fix subscription retention funnels: count renewals on first paid contract
-- (settlements), not re-checkouts as separate contracts. Month k>=2 only after
-- prior period elapsed (30d). Manual grants stay out (no gateway settlements).
-- Return {reached, eligible} per month for eligible-based %.

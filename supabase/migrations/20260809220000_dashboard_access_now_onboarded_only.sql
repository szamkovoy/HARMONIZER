-- Dashboard access_now / users_total: only Harmonizer installers (onboarded_at).
-- CRM email-only imports (membership_tier=free, no onboarded_at) must not inflate
-- «Навигатор» or «Пользователи · всего».
...
begin
  ...
  select jsonb_build_object(
    ...
    'kpi', jsonb_build_object(
      'users_total', (select count(*) from public.users where onboarded_at is not null),

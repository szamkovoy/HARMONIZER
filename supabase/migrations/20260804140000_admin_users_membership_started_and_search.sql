-- membership_started_at for admin «с какого числа»; richer admin_search_users
-- (access filter, activity days, sort).

alter table public.users
  add column if not exists membership_started_at timestamptz;

comment on column public.users.membership_started_at is
  'Start of current membership window (admin override / grant). Null = unknown.';
...
as $$
  ...
  from filtered f
  order by
    case when lower(coalesce(p_order, 'desc')) = 'asc' then
      case coalesce(p_sort, 'created_at')
        when 'onboarded_at' then f.onboarded_at
        when 'tier_end' then f.effective_ends_at
        when 'last_seen' then f.last_seen_at
        when 'last_payment' then f.last_payment_at
    ...
    case when coalesce(p_sort, 'created_at') = 'locale' and lower(coalesce(p_order, 'desc')) = 'asc'
      then coalesce(f.locale, '') end asc nulls last,
    case when coalesce(p_sort, 'created_at') = 'locale' and lower(coalesce(p_order, 'desc')) <> 'asc'
      then coalesce(f.locale, '') end desc nulls last,
    case when coalesce(p_sort, 'created_at') = 'access' and lower(coalesce(p_order, 'desc')) = 'asc'

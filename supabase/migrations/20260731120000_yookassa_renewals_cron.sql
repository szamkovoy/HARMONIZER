-- YooKassa recurring: renew_fail_count + daily cron invoke Vercel renewals runner.

alter table public.payment_contracts
  add column if not exists renew_fail_count integer not null default 0;

comment on column public.payment_contracts.renew_fail_count is
  'Consecutive failed YooKassa autopay attempts; cancel after 2 (or permission_revoked).';
...
revoke all on function public.invoke_run_yookassa_renewals() from public;
grant execute on function public.invoke_run_yookassa_renewals() to postgres;

-- dialog_system_v3 v6 — practice refusal + type/duration coordination (inactive until owner review).
...
- помоги сформулировать контекст дня то есть предстоящие события дня простыми словами, чтобы понять по каждому событию четыре характеристики: что это, примерно когда произойдёт, в какой сфере жизни, а также какие психические состояния или эмоции и личные качества вовлечены;
- не превращай planning в длинный коучинг; тебе достаточно узнать один конкретный предстоящий эпизод за раз;
...
$PROMPT$,
  ...
  1800,
  'text',
  'v6 PREPARED (inactive): practice refusal session lock, type/duration re-ask + confirm-after-yes, catalog ranges 1–5/5–20/>20.'
)

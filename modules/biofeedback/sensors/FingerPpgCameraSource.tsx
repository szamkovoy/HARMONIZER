 ...
 *  - 15 Hz — компромисс той эпохи, когда с камеры ещё пытались снимать
 *    метрики (RMSSD и т.п.) и нужна была плотность RR 67 мс.
 *  - 10 Hz (короткий эксперимент, 2026-07-02) — провал: у `ppg-bandpass`
 *    не было коэффициентов для 10 Hz (нижний bucket — 15 Hz, floor `12`),
 *    поэтому 10-Hz-поток фильтровался фильтром, посчитанным для 15 Hz.
 *    Цифровая полоса «съезжала» к ≈0.53–1.67 Hz и коверкала пульсовую
 *    волну → peak-детектор терял захват (палец на линзе, SQ 0.7–0.88, но
 *    `holding` по 20+ с). Плюс `cadenceScore = scoreRange(fps,12,32)`
 *    штрафовал SQ ниже 12 Hz.
 *  - **12 Hz (эксперимент 2026-07-02) — откат:** при 12 Hz peak-детектор
 *    периодически терял удары на 4–9 с (палец на линзе, SQ 0.69–0.83,
 *    `holding` короткими всплесками), даже после matched-фильтра, дебаунса
 *    `contactLost` и сброса буфера на возврате пальца. Три раунда полевых
 *    тестов подтвердили: 15 Hz reliably держит захват, 12 Hz — нет.
 *  - **10 Hz (эксперимент 2026-07-02) — провал формата:** `useCameraFormat({fps:10})`
 *    на iPhone 14 выбирает более слабый формат камеры с низким max-fps, и камера
 *    начинает отдавать ~6 fps вместо ожидаемых ~10 (тест 1783024540644, algoVer
 *    1.2.4: fps dist 6→321, 7→132). При 6 fps в 12 с окне ~72 сэмпла и
 *    `pickSosForSampleRateHz(6)` клампит к 9 → фильтр FS_10 (рассчитан на 10 Hz)
 *    мисматчит реальным 6 fps → peak-детектор не находит удары, ~75 % сеанса
 *    `holding`. Поэтому **15 Hz reinstated**: `useCameraFormat({fps:15})` выбирает
 *    старший формат, камера отдаёт ~10 fps (auto-exposure cap под torch), и
 *    `pickSosForSampleRateHz(~10)` → matched FS_10.
 *
 * Тепло: при `TARGET = 15` камера реально захватывает ~10 fps (auto-exposure под
 * torch урезает частоту аппаратно), так что нагрев ISP определяется реальными
 * ~10 fps, а не заявленной 15-й. `TARGET = 10` НЕ уменьшал тепло — он лишь
 * переключал формат на более слабый и падал до 6 fps. Если в будущем понадобится
 * дополнительно снизить тепло, путь — adaptive fps / exposure-lock, а не
 * снижение `TARGET` (которое ломает выбор формата).
 *
 * Обоснование 15 Hz (фактическая доставка ~10 fps под torch):
 *  - пульс 40..180 BPM = 0.67..3 Hz → Найквист реальных ~5 Hz даёт ~1.7× запас
 *    (достаточно для ведущего пульса, не для RR-метрик);
 *  - квантование RR ~100 мс несущественно: ведущий пульс сглаживается
 *    по нескольким ударам, точные RR-метрики с камеры не строятся;
 *  - refractory peak-детектора ≥300 мс = ~3 кадра — достаточно для
 *    отсечения double-peak артефактов;
 *  - высокую точность на 20 с активации по-прежнему даём через
 *    `HIGH_PRECISION_PPG_FPS` (30 Hz) — там тепловой бюджет ничтожен.
 */

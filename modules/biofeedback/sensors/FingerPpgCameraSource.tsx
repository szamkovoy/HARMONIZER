 ...
 *    тестов подтвердили: 15 Hz reliably держит захват, 12 Hz — нет. Причина —
 *    низкое разрешение сэмплов (144 сэмпла на 12 с окно против 180 при 15 Hz):
 *    edge-эффект `sosfiltfilt` и адаптивный порог prominence съедают
 *    маргинальные пики. 15 Hz reinstated как проверенный компромисс
 *    надёжность/тепло. Тепло на 20-минутных сессиях — известный tradeoff;
 *    будущая работа: adaptive fps (15 Hz на захвате/ре-локе, 10 Hz в steady-state).
 *
 * Обоснование 15 Hz:
 *  - пульс 40..180 BPM = 0.67..3 Hz → Найквист (7.5 Hz) даёт 2.5× запас;
 *  - квантование RR = 67 мс несущественно: ведущий пульс сглаживается
 *    по нескольким ударам, точные RR-метрики с камеры не строятся;
 *  - refractory peak-детектора ≥300 мс = ~4.5 кадра — достаточно для

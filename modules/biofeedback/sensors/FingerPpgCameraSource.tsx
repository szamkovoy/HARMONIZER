 ...
 *  - **15 Hz (reinstated, затем снова откат до 10 Hz):** сырая оптика-диагностика
 *    (тест 1783020943996, algoVer 1.2.2) показала, что камера реально отдаёт
 *    лишь ~10 fps при включённом torch — автоэкспозиция iOS удлиняет выдержку
 *    на ярком «палец-на-линзе» кадре и аппаратно урезает частоту до ~10
 *    независимо от `TARGET_PPG_FPS`. То есть `TARGET = 15` заставлял ISP
 *    захватывать 15 кадров/с, а JS-процессор обрабатывал ~10 — лишний нагрев
 *    без плотности сэмплов. При `TARGET = 10` захват и обработка совпадают
 *    (~10 fps), нагрев меньше, а matched `BUTTERWORTH_PPG_SOS_FS_10` + floor
 *    `pickSosForSampleRateHz` 9 Hz обеспечивают корректный фильтр для реальной
 *    частоты. Точность RR с камеры всё равно не строится (метрики — только BLE);
 *    ведущий пульс сглаживается по нескольким ударам.
 *
 * Обоснование 10 Hz:
 *  - пульс 40..180 BPM = 0.67..3 Hz → Найквист (5 Hz) даёт ~1.7× запас

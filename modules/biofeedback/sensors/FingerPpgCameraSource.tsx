 ...
 *  - **12 Hz (текущее)** — камера теперь только источник пульса для
 *    ведения дыхания (метрики — исключительно BLE, см.
 *    `ENABLE_FINGER_CAMERA_ADVANCED_METRICS = false`), медицинская
 *    точность RR не нужна. 12 Hz — самая экономная частота, у которой
 *    (а) есть СВОЙ matched Butterworth SOS (`BUTTERWORTH_PPG_SOS_FS_12`),
 *    (б) `cadenceScore` уже без штрафа (порог ровно 12), (в) нагрузка на
 *    ISP / JSI-мост / `analyzeFingerRoi` на ~20 % ниже, чем при 15 Hz.
 *
 * Обоснование 12 Hz:
 *  - пульс 40..180 BPM = 0.67..3 Hz → Найквист (6 Hz) даёт 2× запас;
 *  - квантование RR = 83 мс несущественно: ведущий пульс сглаживается
 *    по нескольким ударам, точные RR-метрики с камеры не строятся;
 *  - refractory peak-детектора ≥300 мс = ~4 кадра — достаточно для

...
/**
 * Окно проверки качества пульса после прогрева (QC). Максимум 20 с: в худшем случае
 * пользователю дают больше времени «нащупать» объектив. Если сигнал стабилен раньше
 * (см. `COHERENCE_QUALITY_WINDOW_EARLY_SUCCESS_MS`), мы выходим из QC сразу.
 */
export const COHERENCE_QUALITY_WINDOW_MS = 20_000;
/**
 * Ранний успех: после этого момента (считая от старта QC) уже делается попытка
 * засчитать проход. Если критерии выполнены — идём в практику, не ждём дальше.
 * Итого короткий путь = 10 с warmup + 10 с QC = 20 с.
 */
export const COHERENCE_QUALITY_WINDOW_EARLY_SUCCESS_MS = 10_000;

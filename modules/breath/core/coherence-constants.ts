...
/**
 * Мост через одиночный аномально длинный RR («провал») на тахограмме.
 *
 * Polar H10 периодически отдаёт один бракованный RR-интервал в несколько секунд (пропущенный/
 * склеенный удар) ПОСРЕДИ ровного потока: поле HR при этом продолжает идти ~непрерывно (пакеты
 * каждую секунду). Такой одиночный «гигантский» интервал (напр. 6044 или 8685 мс при пульсе
 * ~80–120) — это НЕ реальная потеря сигнала на 6–8 с, а артефакт RR-буфера ремня. Без обработки
 * он рвёт тахограмму (> {@link TACHO_MAX_INTERBEAT_GAP_MS}) и раскалывает RSA/когерентность на
 * сегменты, хотя пульс шёл непрерывно.
 *
 * Если разрыв между соседними ударами больше {@link TACHO_MAX_INTERBEAT_GAP_MS}, но НЕ больше
 * {@link ARTIFACT_RR_BRIDGE_MAX_MS}, мы восстанавливаем недостающие удары, деля интервал на
 * `round(gap / localMedianRr)` равных частей по локальному медианному RR. Так тахограмма остаётся
 * непрерывной (RSA в этом отрезке ≈ 0 — честно: детали beat-to-beat потеряны), а метрики не
 * распадаются на куски. Разрывы больше порога — это уже настоящая потеря контакта (снятый ремень,
 * оптический «палец убран»): их НЕ восстанавливаем, они остаются честной дырой + серой полосой.
 */
export const ARTIFACT_RR_BRIDGE_MAX_MS = 12_000;
...
export const COHERENCE_ALGORITHM_VERSION = "1.2.23";

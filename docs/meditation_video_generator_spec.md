...
- единый `MandalaVisualCore` на `Skia`;
...
flowchart LR
    ...
    BioMixer --> VisualCore[MandalaVisualCore]
...
`MandalaVisualCore` не делится на отдельные runtime-модули `generator` и `shader`, потому что форма, кинетика, цвет, священные пресеты и live-модуляция вычисляются в одной графической системе.
...
- дыхание: `0.33 Hz`;
- пульс: `1.10 Hz`;
- `hrv`: среднее нормализованное значение `0.5–0.6`;
...
4. Далее `MandalaVisualCore` работает уже с runtime-состоянием.
...
Оба источника должны отдавать один и тот же контракт `BioSignalFrame`. Это критично, чтобы замена симуляции на реальный PPG не требовала переписывать `MandalaVisualCore`.
...
- `hrv` и `stressIndex` плавно возвращаются к средним значениям;
...
- В проекте есть отдельный модуль `MandalaVisualCore`.

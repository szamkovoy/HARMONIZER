...
## `marketing_email` / клики по конкретным ссылкам (2026-09-15)

- **Контекст:** first-party click пишет `payload.click.link`, но unique `(resend_id, event_type)` сохраняет только первый клик, а KPI на карточке — сумма «Клики».
- **Проявление:** не видно, YouTube это, Дзен или VK.
- **Действие:** отдельный отчёт по URL, если понадобится после первой волны.

## `marketing_email` / синхронная отправка большой рассылки (2026-09-14) — закрыто 2026-09-15

- Волны + queued drain + halt: `emailCampaignSend`, cron `/api/cron/email-campaigns`, миграция `20260915184713`.

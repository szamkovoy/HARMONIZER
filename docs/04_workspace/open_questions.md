...
- **Контент интро-экранов онбординга (прогрев):** каркас в `app/onboarding.tsx` готов (шаг `warm`, spinner + prefetch), количество экранов и контент пришлёт продукт.
- **Платёжная система (Lava.top подключена 2026-07-15):** подписки «Наставник»/«Мастер» работают (checkout + вебхуки + отмена, см. `account_web/spec.md` §3.1). Открыто: (a) разовая оплата вебинара (`ctx="webinar:<id>"` уже прокидывается в кабинет); (b) курсы/интенсивы (заготовка `COURSES` в `cabinet.html` — нужны офферы ONE_TIME в Lava и расширение checkout на произвольный offerId); (c) годовая периодичность (в Lava достаточно включить цену «Год» в оффере; в кабинете добавить выбор месяц/год); (d) ответ поддержки Lava о параллельных активных контрактах одного клиента на разных офферах — наша механика уже отменяет старую подписку при активации новой, но подтвердить поведение Lava; (e) Lava-вебхук пишет в `payment_contracts`/`users.membership_*` напрямую, минуя админ-леджер `payments` — решить, нужна ли дублирующая запись в леджер для сквозной истории.
...
- Dual model Анонс/Запись реализован (2026-07-13). Открыто: (a) одноразовая оплата вебинара для free; (b) пуши «скоро» / «запись доступна» на сегмент `webinar_registrations`; (c) схлопывание тарифов Free/Oracle/Master vs Практик.

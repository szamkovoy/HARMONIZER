...
- **Android Google Maps API key for «Проверить на карте» (2026-07-24):** код читает `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY` в `app.config.ts` → `android.config.googleMaps.apiKey`. Ключа в `.env.local` пока нет — без него Pixel падает с `API key not found` при открытии `BirthPlaceMapModal`. **Действие:** создать ключ Maps SDK for Android в Google Cloud, добавить в `.env.local`, сделать Android native rebuild (EAS/dev-client). iOS использует Apple Maps и ключ не требует.

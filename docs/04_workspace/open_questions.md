...
## `assistant` / йога и практики как действия дня без practice-ветки (2026-09-14)

- С 2026-09-14 в flow **без** ветки practice (Day-tab «Добавить действие», Oracle/Free home/plan, day-summary) действия вида «Занятия йогой» / «Медитация» / «Дыхание» **сохраняются** как обычные действия дня (раньше молча отфильтровывались как дубликат карточки практики — 4 диалога Лизы с пустым финалом). В Master/trial flow с practice-веткой фильтр сохранён. Открыто продуктово: (а) должна ли Day-карточка «Занятия йогой» у Oracle/Free мягко вести в каталог практик; (б) нужно ли у Master в add-flow (тоже без practice-ветки) вместо сохранения действия предлагать выбрать практику из каталога. Код сейчас: сохраняем как действие, без отсылок.
- Наблюдение из QA-разбора (не исправлено кодом): модель на finalize иногда переносит `display_order`/ярлык так, что карточка «Дня» получает формулировку из финала, а не из первого упоминания. Пока приемлемо; отслеживать по `planningPersistence.updated` в архиве.

## `marketing_email` / синхронная отправка большой рассылки (2026-09-14)

- **Контекст:** `POST /api/admin/email/campaigns/[id]/send` шлёт получателей в одном Vercel-инвоке (`maxDuration` 300s) с паузой 500 ms на письмо. После фикса выборки сегмент «Вся база» — ~15k active contacts.
- **Проявление:** полный сегмент не укладывается в 300s (~600 писем максимум за инвок). Таймаут оставляет кампанию в `sending` с частично отправленными письмами.
- **Действие:** очередь/батчи (продолжение send по уже queued `email_campaign_sends`) или отдельный worker, если разовые рассылки на всю базу станут штатными.

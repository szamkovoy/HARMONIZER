...
- **Платёжная система:** страница кабинета содержит точку входа `hzStartUpgrade(tier)` (заглушка). Подключение провайдера, вебхук на Vercel (запись в `payments` + `users.membership_*`) и разовая оплата вебинара — после готовности кабинета.

...
- Список `/admin/payments` и история в карточке пользователя → одна и та же модалка/`POST /api/admin/payments/refund`.
...
- **ЮKassa API:** preflight `GET /v3/payments/{id}` → `POST /v3/refunds`; статус платежа и тариф меняются **только** при `status=succeeded`. `pending`/ошибка → без изменений в БД; в модалке кнопка «Сделать возврат вручную» (`yookassa_mark`, как Lava). Вебхук `refund.succeeded` тоже помечает контракт и снимает тариф.
- **Смена shopId:** платежи старого магазина API текущего магазина не видит (часто сырое «Refund not found or forbidden»). Возврат денег — в ЛК **того** магазина, затем «Сделать возврат вручную».
- **Баланс магазина:** при `Insufficient funds` ЮKassa не создаёт возврат — пополнить баланс или вернуть в ЛК ЮKassa, затем mark.
- После успешного возврата подписки: лучший ещё действующий тариф среди оставшихся active/cancelled subscription-контрактов **и** ручных грантов; иначе `free`. Это не cancel-автопродления: доступ по возвращённому платежу снимается сразу.

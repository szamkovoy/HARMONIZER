...
- `app.json` plugins включают `expo-font` и `expo-web-browser` (autolinking config для SDK 54); `expo-splash-screen` — `enableFullScreenImage_legacy: true` (iOS full-bleed) + `imageWidth: 288` + `resizeMode: cover` + `backgroundColor: #f7f7f7`. Android 12+: native centered icon остаётся (прозрачная иконка убрана — давала белый hang). До шрифтов — `EarlySplashCover` (absoluteFill, hide native после `Image.onLoad`); далее `AppStartupProvider` держит splash пока `initializing || (session && profileLoading && !profile)` и не сбрасывает blocking из‑за пустых segments.

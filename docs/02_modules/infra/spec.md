...
- `app.json` plugins включают `expo-font` и `expo-web-browser` (autolinking config для SDK 54); `expo-splash-screen` — `enableFullScreenImage_legacy: true` (iOS full-bleed) + `imageWidth: 288` (Android icon) + `resizeMode: cover`. До шрифтов `EarlySplashCover` показывает тот же `splash.png` cover и снимает нативный splash (`app/_layout.tsx`).

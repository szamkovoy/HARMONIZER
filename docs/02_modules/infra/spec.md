...
- `app.json` plugins включают `expo-font` и `expo-web-browser` (autolinking config для SDK 54); `expo-splash-screen` — `enableFullScreenImage_legacy: true` (iOS full-bleed) + `imageWidth: 288` + `resizeMode: cover` + `backgroundColor: #ffffff`. `EarlySplashCover` (явные `useWindowDimensions` + cover) на iOS и Android до handoff к `AppStartupSplashOverlay` (`onSplashReady`); на iOS — лёгкий `iosMatchVeil`. Android 12+: **видимая** centered splash-icon (`with-android-splash-hide-icon` отключён — прозрачный icon = белый hang); hide native после paint JS cover. `RootLayout` ждёт `useThemePreference().ready`; root/Stack `contentStyle` / gate = `theme.colors.screenBg`. Пустые Expo `segments` на cold start считаются Home — splash не снимается на пустой underlay. `AppStartupProvider` держит splash пока `initializing || (session && profileLoading && !profile)` или Home bootstrap blocking. Корень: `GestureHandlerRootView` в `app/_layout.tsx`.

...
- `app.json` plugins включают `expo-font` и `expo-web-browser` (autolinking config для SDK 54); `expo-splash-screen` — `enableFullScreenImage_legacy: true` (iOS full-bleed) + `imageWidth: 288` + `resizeMode: cover` + `backgroundColor: #ffffff`. `EarlySplashCover` (явные `useWindowDimensions` + cover) на iOS и Android до handoff к `AppStartupSplashOverlay` (`onSplashReady`); на iOS — лёгкий `iosMatchVeil`, чтобы первый кадр не был ярче AppStartup. Android 12+: `plugins/with-android-splash-hide-icon.js` (`withFinalizedMod`) — прозрачный `windowSplashScreenAnimatedIcon`; hide native только после `Image.onLoadEnd`. `AppStartupProvider` держит splash пока `initializing || (session && profileLoading && !profile)`. Корень: `react-native-gesture-handler` + `GestureHandlerRootView` в `app/_layout.tsx`.

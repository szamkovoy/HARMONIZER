...
- `app.json` plugins включают `expo-font` и `expo-web-browser` (autolinking config для SDK 54); `expo-splash-screen` — `enableFullScreenImage_legacy: true` (iOS full-bleed) + `imageWidth: 288` + `resizeMode: cover`. Android 12+: `plugins/with-android-splash-hide-icon.js` (`withFinalizedMod`) ставит прозрачный `windowSplashScreenAnimatedIcon` (системный splash = только фон). `EarlySplashCover` держится поверх до `AppStartupProvider.onSplashReady`; JS-заставка — full-bleed cover на opacity 1 (без dim «дыхания» картинки на белом), лёгкий shimmer остаётся.

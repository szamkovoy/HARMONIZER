...
- **Данные:** версионируемая схема и edge-функции живут в `supabase/`; продакшен-изменения проходят через новые SQL-миграции, как зафиксировано в README. Для `author_presence` появился отдельный cleanup-контур `cleanup-expired-stories`, который удаляет истёкшие сторис и их storage assets вне интерактивного клиентского пути.

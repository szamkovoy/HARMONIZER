...
- `app.json` plugins включают `expo-font` и `expo-web-browser` (autolinking config для SDK 54); `expo-splash-screen` — `enableFullScreenImage_legacy: true` (iOS full-bleed) + `imageWidth: 288` + `resizeMode: cover` + `backgroundColor: #ffffff`. iOS до шрифтов: `EarlySplashCover` (`useWindowDimensions` + cover). Android: без EarlySplashCover — native icon до `AppStartupSplashOverlay` Image.onLoad (явные width/height; absoluteFill давал растяжение). `AppStartupProvider` держит splash пока `initializing || (session && profileLoading && !profile)`.

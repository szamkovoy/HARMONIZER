...
- **2026-08-23 (OTP email — Amazon SES via raw HTTP + SigV4):** Registration hung: `signInWithOtp` never returned (40s+, 0 bytes), app stuck on «Отправляется..». Root cause: `mail/send.ts` imported `./providers/ses.ts` (→ `npm:@aws-sdk/client-sesv2` v3) **statically at module top level**. The multi-MB AWS SDK v3 module tree exceeds the Supabase edge-runtime cold-start budget, so the `send-auth-email` auth hook failed to respond within GoTrue's hook timeout → `signInWithOtp` hung. Amazon SES itself is healthy and out of sandbox (delivers to `sezam777@yandex.ru`); the failure was purely the SDK cold-start, not SES. Fix (Amazon SES retained as the OTP provider): (1) `send.ts` now lazy-imports `./providers/ses.ts` only on the `AMAZON_*` path so the Resend cold start stays lightweight; (2) `providers/ses.ts` rewritten from `@aws-sdk/client-sesv2` to a lightweight raw HTTP `POST /v2/email/outbound-emails` signed with **AWS Signature V4** (service `ses`, IAM keys used directly — same auth path the SDK used, no SMTP-password derivation), with a 12s `AbortController`; (3) `sendViaResend` also got a 12s `AbortController`. Verified on Supabase with real SES creds: `SendEmail` → 200 + `MessageId` in ~0.9s. `EMAIL_OTP=AMAZON_ZAMKOVOI_YOGA` kept on Supabase + `.env.local`. The heavy `@aws-sdk/client-sesv2` dependency is now gone from the OTP edge path.

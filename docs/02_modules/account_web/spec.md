...
| `DELETE /api/account/subscription` | Bearer кабинетной сессии | Отмена активных subscription-контрактов через `cancelActiveSubscriptionsForUser` (диспетчер по `provider`; сейчас `lavatop` → Lava DELETE) + статус cancelled; доступ у пользователя остаётся до `current_period_end`. Ответ `{ cancelled, accessUntil }` |
...
- **Страница кабинета** (`web_cabinet/cabinet.html`): standalone HTML5 (`<!DOCTYPE html>`, viewport, charset, Hanken Grotesk через Google Fonts, `theme-color`). Рендер под макет (шапка, «Гармонизатор», «Другие тарифы», «Дополнительно»). Оферта — `GET {API_BASE}/cabinet/offer/{lang}.json` только по клику. **Boot:** мини-скрипт сразу ставит «Загружаем…» и стартует `POST /session`; контент — только после полного `overview`. **Деплой страницы:** загрузить файл на хостинг (ISPManager) как `https://zamkovoi.yoga/cabinet/index.html` **без** темы WordPress; плюс Vercel для API и `offer/*.json`. CORS: `ACCOUNT_CABINET_ALLOWED_ORIGIN=https://zamkovoi.yoga` (path не важен).

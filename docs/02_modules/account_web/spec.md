...
- **`MembershipEventsBridge`**: (а) Realtime UPDATE своей строки `users` → `refreshProfile()`; (б) foreground — выборка membership-полей и `refreshProfile()` только при изменении отпечатка; (в) сравнение `baseTierFromRow` с сохранённым `lastTier` → модалка повышения уровня («{from} → {to}», `gate.tierChanged.*`); (г) `trial_expires_at` в прошлом при free-базе → одноразовая модалка `gate.trialEnded.*` (кнопки «Закрыть» / «Личный кабинет»); (д) foreground-проверка `cabinetVisit` с `ctx=webinar:<id>` → если появилась регистрация на вебинар, модалка `gate.webinarPaid.*` (ретрай 10с); (е) foreground-проверка последней one_time-покупки через `GET /api/account/purchases/last` → если книга оплачена после визита в кабинет, модалка `gate.bookPaid.*` (ретрай 10с; флаг `bookThanksShown.<uid>.<contractId>`).

...
- **Realtime на `users`** — требует, чтобы строка оставалась в publication `supabase_realtime` и RLS-select своей строки не сузился; иначе подхват уровня деградирует до foreground-refetch. С 2026-09-14 подписка **не постоянная**: канал живёт 30 мин после `markCabinetVisit` (`openAccountCabinet.ts` → `subscribeCabinetVisit`), поэтому любой новый call-site, который ведёт пользователя платить, обязан проходить через `markCabinetVisit` — иначе мгновенного подхвата не будет (останется foreground-refetch).

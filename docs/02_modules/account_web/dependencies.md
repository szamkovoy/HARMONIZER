...
- **ЮKassa (внешний)** — `api.yookassa.ru/v3`: create payment (redirect) + GET payment из вебхука; уведомления на `/api/account/webhooks/yookassa`. Цены RUB — `payment_catalog`. Выбор шлюза — `selectPaymentProvider`. Fee settlement 2.5% (`yookassa` / `YANDEX_GATEWAY_FEE_RATE`).
...
- **Цены в `cabinet.html`** — из overview: Lava products или `payment_catalog` (ЮKassa/RUB). Смена Lava-цены — кэш 10 мин; смена ЮKassa — SQL/`payment_catalog`.

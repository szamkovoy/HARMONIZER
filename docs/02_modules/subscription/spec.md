...
- **Типы и константы:** `FeatureKey`, `ProductTier`, `PaidProductTier`, `EffectiveAccess`, `AccessContextValue`, `TIER_FEATURES`, `FEATURE_REQUIRED_TIER`, `PRODUCT_TIERS`, `PAID_PRODUCT_TIERS`, `TIER_LABELS`, `TIER_LABELS_RU`, `TIER_ORDER`, **`tierAtLeast(tier, minimum)`**, **`isPaidProductTier`**, **`isProductTier`**.
- **`DevTierSwitch`** — UI переключателя эффективного тарифа в dev (`__DEV__`).
- **`UpgradeDialog`** — модалка апгрейда по `FeatureKey` + `requiredTierFor`; подписи тарифов и фич через **`useTranslate()`** (`tier.*`, `upgrade.*` в JSON-каталоге), не `TIER_LABELS`. Админ-панель использует **`TIER_LABELS_RU`** из того же `tiers.ts`.
...
**План:** оплата store не подключена — тариф назначается вручную через админ-леджер `payments`; при истечении срока hourly cron пересчитывает membership (без автооплаты). Возможное переименование тарифов (Навигатор/Советник/Учитель и т.п.) — только через канон `tiers.ts` + миграция CHECK. Ориентир по продукту и навигации: `docs/tmp_docs/02052026/access_tiers_navigation_brief.md` (архивная копия в `docs/05_archive/old_briefs/`). Методическая модель тарифов и терминология **free/premium в UI** — в справочнике ниже.

...
**План (Consumption-Only, 2026-07-14):** оплата в сторах не подключается — вся коммерция на внешнем сайте `zamkovoi.yoga` через OTT-переход «Личный кабинет» (модуль `account_web`). Тариф назначается вручную через админ-леджер `payments` или (после подключения платёжной системы) вебхуком провайдера; при истечении срока hourly cron пересчитывает membership. Смена уровня подхватывается приложением мгновенно (Realtime на `users` + foreground refetch — `MembershipEventsBridge`, см. `account_web/spec.md`). Методическая модель тарифов и терминология **free/premium в UI** — в справочнике ниже.

...
- **`AffirmationBreathOverlay`** — intro/финал: hint как в конце; голос и панель появляются **ровно с началом выдоха** (не за секунду до); fade-in ~280 ms / fade-out ~1 с + adaptive edge-trim; audio **warm** (disk cache + preloaded `Sound`) при загрузке / входе в finale, чтобы cue не ждал сеть; панель растворяется с `dimOpacity` практики; окно финала ≈4.2 цикла / 3 play.
...
- «Последние 3 выдоха» оцениваются по remaining time / msExhaleInterval (cycleMs / exhalesPerCycle от плана); практика time-based. Для nadi-shodhana (2 выдоха на цикл планировщика) интервал выдохов = cycleMs/2, поэтому финал стартует в 2× позже, чем у практик с одним выдохом на цикл.

...
- `useDayContent(options?)` → `UseDayContentResult` (`modules/home/useDayContent.ts`):
  - `forecast: DailyForecast | null`, `accessMode`, `modelUsed`, `source` (`"cache" | "computed" | "global"`), `status`, `loading`, `error`, **`locationIssue: LocationAcquireFailureReason | null`** (причина сбоя авто-GPS; см. `modules/location/acquireAndPersistUserCoordinates.ts`), **`userLocation: DayContentUserLocation | null`** (`{ lat, lng, timezone }` из профиля при наличии координат), `refresh(opts?)`. Экспорт типа **`DayContentUserLocation`**. Опции **`refresh`** типизированы экспортом **`DayContentRefreshOptions`**: `forceRefresh?`, **`localeChange?`** (смена языка — фоновый refresh без blocking splash), `accessModeOverride?`, `accessTierOverride?`, **`blockingReload?`** (при `true` — стартовый оверлей до готовности дня после смены натала с другого экрана; см. `services/homeDayContentReloadRequest.ts` + **`useFocusEffect`** на главном в `app/(tabs)/index.tsx`). При смене локали хук подписан на **`subscribeAppLocale`**: сразу сбрасывает locale-specific LLM-поля через **`stripHomeLlmTexts`**, инвалидирует ключи monologue/hydration и вызывает `refresh({ forceRefresh: true, localeChange: true })` (логика перенесена из `app/(tabs)/index.tsx`).

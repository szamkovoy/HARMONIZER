...
- `useDayContent(options?)` → `UseDayContentResult` (`modules/home/useDayContent.ts`):
  ...
  - Персональный режим больше не держит первый рендер до полной LLM-генерации: если базовый `DailyForecast` валиден, home может открыться сразу; вторичный слой (`slogan`, `recommendationShortText`, `recommendationLongText`, `mathLevel`) догружается в фоне через `callMonologue("morning_recommendation", …)` и затем перезаписывает клиентский day-cache. После каждого успешного HTTP-ответа `daily-forecast` UI получает forecast через **`stripHomeLlmTexts`** (без stale LLM-текстов) и помечает **`pendingMorningMonologueForce`**, чтобы monologue всегда запрашивался с `variables.forceRefresh: true`, даже если серверная строка ещё содержит старые recommendation-поля (смена натала/локали). Ключ фоновой hydration включает **`scopeKey`** (с суффиксом локали, см. §4).
...
- **Таймауты транспорта:** `DAILY_FORECAST_TIMEOUT_MS` в `dailyForecastClient.ts`; для free-ветки `GLOBAL_CONTENT_TIMEOUT_MS` (**25s**) в `globalContentClient.ts` ограничивает HTTP-запрос к `global-content` и fallback-read из `global_daily_content` (любая локаль); **`DAY_PLAN_FETCH_TIMEOUT_MS` = 45s** в `services/dayPlan.ts` для GET `/api/day`; авто-GPS на home — **`LOCATION_ACQUIRE_TIMEOUT_MS`** в `modules/location/acquireAndPersistUserCoordinates.ts`.

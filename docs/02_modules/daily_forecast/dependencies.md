...
- **`astro` (типы и движок)**  
  ...
  - **Клиентский UI home → daily-engine:** `modules/home/ui/OpportunityWindows.tsx` импортирует `samplePlanetAltitudeForDay`, `interpolateDiurnalAltitude` из `modules/daily-engine`; `freeWindows.ts` делит `planetAltitudeAt` с `planetDiurnalCurve.ts`. Суточная кривая на карточке — `react-native-svg` (`Svg`, `Polyline`).

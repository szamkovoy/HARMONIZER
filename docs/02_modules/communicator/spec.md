...
`**DialogCompleteEvent**` — кроме `fullText` / `turnMode` / `practicePicked` / `messageId` может нести контекст ветвления и persistence: `branches`, `phaseTime`, `targetChakra`, **`planningPersistence`** (`inserted` / `summarized` / `skipped`), `relatedEventIds`, `skippedPlannedEvents`, `matrixCells`; клиент сохраняет эти поля в `messages.meta` и dev-export. SSE `complete` приходит **после** persist assistant-row на сервере (см. `assistant/spec.md`).

...
- При session hydration в `Communicator.tsx`: если server `content` пустой, но в `meta` есть `practicePicked` и final-like `turnMode`, а локальный `currentText` непуст — гидрация считается успешной (lean storage).

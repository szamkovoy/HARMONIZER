...
4. **Карточка практики** — `Communicator` рендерит общий `modules/practices/ui/PracticeCard.tsx`, переводя серверный `PracticePicked` в `PracticeSummary`. Текст описания в карточке приходит с сервера уже **кратким** (`reason` в `practice_picked`, см. `assistant` / `practiceCardSummary.ts`). Для **йоги** `overrideDurationMinutes` / `overrideChakraIndex` **не** передаются (значения каталога); для breath/meditation пользователь может переопределить duration/chakra перед запуском; затем вызывается `launchPractice(..., { launchSource: 'assistant' })`.
...
- `practicePicked` → `meta.practicePicked` и общий `PracticeCard`; для **дыхания/медитации** в DTO может быть `**overrides: { durationMin, chakraIndex }`** (маркер / вывод истории); для **йоги** сервер `**overrides`** не присылает — `Communicator` не прокидывает override-пропсы в `PracticeCard`. Опционально `markerIdResolved: false` (если model-generated id не найден в каталоге). Поле `**reason**` — краткий текст карточки с сервера (`practiceCardSummary.ts`), не длинный текст из маркера модели.

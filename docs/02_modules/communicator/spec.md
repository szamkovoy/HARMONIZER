...
- `fetchDialogSession({ useCase, entrySource, scenarioId?, conversationId?, debugExport?, signal? })` — GET синхронизации сессии; опционально `debugExport: true` → query `debugExport=1` и поле `dialogStateAfter` в ответе при серверном test/debug режиме; опционально `conversationId` запрашивает конкретную беседу по id, даже если она уже закрыта, что используется dev-export'ом для канонической выгрузки завершённых диалогов. При ошибке «нет эндпоинта» для старых серверов возвращается пустая сессия с `reset: true`.
...
5. **Dev export** — в `__DEV__` показывается кнопка `Export dialog to JSON`, которая собирает `day_context` из `triggerMeta`, верхнеуровневый блок `conversation` (`conversation_id`, `use_case`, `entry_source`) и метаданные сообщений (`turnMode`, `modelTier`, `validation`, `practicePicked`, `branches`, `phaseTime`, `targetChakra`, `insightMetrics`, **`planning_persistence`**). Перед экспортом клиент пытается взять каноническую server session sync через `fetchDialogSession({ debugExport: true })`; если она доступна и совпадает с текущей беседой, клиент **не выбрасывает локальные реплики слепо**, а мерджит их с server-sync по порядку и совпадающему `role + text`. Поэтому export сохраняет **весь видимый текст диалога**, даже если server session sync короче локального списка, но при этом всё равно подтягивает серверные `meta`, где они есть. `conversation.canonical_source` теперь может быть не только `server_session_sync` / `local_client_state`, но и `server_session_sync_merged_with_local`, когда серверный снимок использован как основа, а недостающие сообщения добраны из клиента. При активном server test/debug режиме export также включает `dialog_state_after` (снимок forecast/planned_events/daily_matrix + context/summary через GET `?debugExport=1`), а внутри него — **`planning_snapshot_at_start`**, **`planning_open_now`**, **`planning_closed_recent_48h`**, **`planning_created_in_this_conversation`** для проверки planning/summarizing веток.

...
`export function Communicator(props: CommunicatorProps): JSX.Element`
  ...
  - `onPracticePicked?: (practice: PracticePicked) => void | Promise<void>` — вызывается после успешного локального `launchPractice(...)` и сигнала готовности экрана практики (`signalAssistantPracticeScreenMounted`); модальные поверхности используют его как side-effect закрытия оверлея/refresh без промежуточного flash вкладки под ним.
...
4. **Карточка практики** — `Communicator` рендерит общий `modules/practices/ui/PracticeCard.tsx`, переводя серверный `PracticePicked` в `PracticeSummary`. Текст описания в карточке берётся из `**practicePicked.card_blurb`** (если сервер прислал валидированный model-generated blurb или fallback summary) либо из `**reason`** как legacy fallback; новый серверный контракт держит `reason` как текст связи практики с днём, а `card_blurb` как текст карточки о самой практике. Для **йоги** `overrideDurationMinutes` / `overrideChakraIndex` **не** передаются (значения каталога); для breath/meditation пользователь может переопределить duration/chakra перед запуском; **`PracticeCard`** клипит `overrideDurationMinutes` к допустимым шагам каталога (см. `assistantSelectableDurations.ts`, тот же контракт, что и сервер в `resolvePracticePublic`). При нажатии `Начать практику` компонент сначала вызывает `launchPractice(..., { launchSource: 'assistant' })`, а `onPracticePicked` — через `scheduleAssistantOverlayDismiss` (`modules/practices/ui/assistantPracticeOverlayDismiss.ts`) как side-effect закрытия overlay/refresh: callback срабатывает после `signalAssistantPracticeScreenMounted()` из route-обёрток практики (`useAssistantPracticeOverlayDismiss`, `launchSource=assistant`) **и** min-delay **700 ms**, либо по fallback timeout **2500 ms**, чтобы root-stack экран успел смонтироваться и отрисоваться до dismiss full-screen Modal и не мелькала вкладка «День». Home `CommunicatorOverlay` держит overlay смонтированным (`communicatorMounted`) отдельно от видимости (`Modal.visible` + `animationType`: `"none"` при старте практики, `"slide"` при обычном закрытии); `onDismiss` размонтирует overlay после анимации. Если navigation не удалась, callback вызывается сразу как fallback. `practiceToSummary` запекает `practicePicked.overrides.durationMin` в pending `PracticeSummary` для дыхания/медитации (отложенная карточка на вкладке «День» показывает выбранную длительность, а не каталожный дефолт).

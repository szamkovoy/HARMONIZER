...
`export function Communicator(props: CommunicatorProps): JSX.Element`
  ...
  - `startFreshSession?: boolean` — если `true`, компонент при первом boot этого mount не делает session-resume по `entrySource`, очищает local session-cache для этого ключа и стартует новую беседу с пустой лентой. Повторные rerender внутри того же открытого чата **не должны** снова сбрасывать ленту. Используется Home-кнопкой `Что делать?` и day-modal flow (`plan` / `add` / `summary`), чтобы вкладка `День` всегда открывала отдельный чистый диалог под текущий режим, а не resume старой day-сессии.

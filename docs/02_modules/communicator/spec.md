...
`export function Communicator(props: CommunicatorProps): JSX.Element`
  ...
  - `startFreshSession?: boolean` — если `true`, компонент на mount не делает session-resume по `entrySource`, очищает local session-cache для этого ключа и стартует новую беседу с пустой лентой. Используется Day add-flow и Home-кнопкой `Что делать?`, когда продуктово нужен новый daily dialog, а не resume старой home-беседы.
...
- `sendDialogMessage(params: SendDialogMessageParams): Promise<SendDialogMessageResult>` — POST с телом JSON, ответ читается как **SSE**: на **web** — `fetch` + потоковое чтение `body`, на **iOS/Android** — `**XMLHttpRequest**` с инкрементальным `responseText` (иначе RN часто отдаёт весь поток одним куском в конце). JSON-ошибки с object-valued `error/message/details` сериализуются в читаемый текст, чтобы dev LogBox не показывал `[object Object]`. `DialogueEntrySource` включает `"day"` для модального ассистента вкладки «День»; этот источник использует тот же daily dialog, но передаёт `triggerMeta.dayTabMode`, `workingLocalDate`, `daySummaryRequested`, `dayActions`, `dayPractices` и, при summary-ветке, временный `dayHealthContext`. Home может передавать тот же `dayHealthContext`, если auto-summary запускается прямо с главной по незакрытому прошлому дню. `dayHealthContext` собирается перед auto-send: включает йогу за день и 7-дневное среднее по `user_daily_stats` (как именно дневное среднее, включая нулевые дни, а не только «дни с практикой»), плюс мягкий ориентир достаточности около `30` минут в день, а также реальные native-метрики из `services/nativeHealth.ts` — Apple HealthKit на iOS, Google Health / Android Health Connect на Android. Для summary-ветки вкладки «День» backend теперь принудительно ведёт пользователя по одному due-событию за ход, чтобы подытоживание не смешивало несколько эпизодов в одно сообщение. Запрашиваются только read-права на steps, active calories, exercise/workouts и sleep; при отсутствии provider или отказе пользователя поля остаются пустыми, а повторный permission prompt идёт по мягкому backoff `1 день -> 7 дней -> 30 дней`. `complete` теперь может нести `latencyMs`, а `Communicator` сохраняет его в `messages.meta`/dev-export для QA-диагностики долгих turn-ов. `Communicator` также отдаёт `onPracticeOffered(PracticeSummary)` при появлении assistant `practicePicked`, чтобы внешняя поверхность могла сохранить карточку как pending offer до запуска практики; Home и Day используют это одинаково. Если финальный ход завершает разговор **без** карточки, компонент показывает под последним сообщением явную кнопку выхода и перед закрытием делает best-effort `reconcileDialogPlans({ force: true })` (серверный endpoint — совместимый no-op с FSM).

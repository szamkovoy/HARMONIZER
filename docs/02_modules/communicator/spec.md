...
3. **Стрим ответа** — `runCommunicatorStream` → `sendDialogMessage` парсит SSE-блоки (`parseSseBlock`) и для событий `orchestrator_decision`, `chunk`, `complete` обновляет состояние. На нативных платформах чанки чаще приходят по мере генерации (**XHR** + polling `responseText`); при `xhr.onerror` после закрытия сокета клиент **финализирует** буфер и **resolve**, если уже есть `complete` или непустой `fullText` (типичный iOS/RN артефакт при успешно принятом теле). Список сообщений — `**@shopify/flash-list` (`FlashList`)**; активный стрим — последняя строка данных `kind: "stream"`. Пока текста нет — в пузыре `**ActivityIndicator**`; с первого символа после `stripStreamingMarkers` + `**stripDialogScaffoldMarkdown**` (убирает `---` и **целиком** блоки `**…**`, без разворачивания в видимый текст) — `**StreamingAssistantLines**`: строки по `\n` с лёгким `**FadeIn**` (`react-native-reanimated`), последний незавершённый сегмент обновляется по мере чанков; курсор **▍** при `streamStatus === "typing"` и непустом хвосте. Финальный коммит в историю — через **`commitAssistantTurn`** (общий для `submitDialogTurn` и `initiateDialog`). **Добавление финального сообщения в `messages` и `resetChatStream()` откладываются** до совпадения stripped-текста с целевой строкой (`pendingRevealGoal`) + короткая задержка в компоненте, иначе карточка практики и финальный пузырь «съедали» бы анимацию. Этот отложенный путь включается только если агрегат стрима **`mergedText`** после trim **непуст**, ответ **не** восстановлен одной попыткой **`fetchDialogSession`** при пустом стриме, длина после strip > **14**, и в **`complete` нет `practicePicked`**; если **`practicePicked` пришёл** — коммит **сразу** (карточка не должна ждать reveal). Перед отложенным коммитом **`syncDisplayText(finalText)`** выравнивает live-стрим с sanitized `complete.fullText`, иначе `stripTarget` (сырые SSE-чанки) и `revealGoal` (серверный sanitized) расходятся и flush не срабатывает до таймаута **60 с**. Очень короткие ответы (порог после strip, **14** символов) коммитятся сразу; таймаут принудительного коммита **60 с**. Сообщение-заглушка «не получил полный текст» **никогда** не проходит через отложенный коммит (раньше длинный fallback ошибочно ждал до **60 с**). Если агрегат стрима пуст **или** `submitDialogTurn` завершился исключением, выполняется **одна** попытка подтянуть последнюю реплику ассистента через `**fetchDialogSession**` (~700 ms) — на случай, когда сервер уже сохранил ответ, а SSE оборвался; в синтетический `complete` для `**commitAssistantTurn**` из `meta` последнего assistant-сообщения копируются `practicePicked`, `turnMode`, `modelTier`, `modelUsed`, `iteration`, `debug` (как `debugExport`), чтобы карточка практики и dev-бейдж не терялись при восстановлении. Дополнительно `commitAssistantTurn` теперь делает **тот же session hydration и на success-path**, если текст уже есть, но `complete` отсутствует или неполон (`turnMode` / `modelTier` / `modelUsed` / `iteration` / `practicePicked` на финальном ходе): это закрывает случай «финальный текст дошёл, а `complete` потерялся», при котором раньше в истории оставались `null`-метаданные и карточка не рендерилась. При `complete` итог — `complete.fullText` (sanitized); иначе агрегат SSE после `stripStreamingMarkers` + `stripDialogScaffoldMarkdown`; пусто — `emptyAssistantReplyFallback`.
...
5. **Dev export** — в `__DEV__` показывается кнопка `Export dialog to JSON`, которая собирает `day_context` из `triggerMeta` и метаданные сообщений (`turnMode`, `modelTier`, `validation`, `insightMetrics`) и отдаёт их через RN `Share`. При активном серверном test/debug режиме (`TEST_MODE_FAST_INTERVALS=1` или `DEBUG_DIALOG_EXPORT=1`) в каждое assistant-сообщение добавляется блок `debug` (маркеры, phase/branches/due_events/practice_pick), в конец JSON — `dialog_state_after` (снимок forecast/planned_events/daily_matrix через GET `?debugExport=1`).

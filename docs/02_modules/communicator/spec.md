...
- `onPracticePicked?: (practice: PracticePicked) => void | Promise<void>` — вызывается перед локальным `launchPractice(...)`; модальные поверхности используют его, чтобы закрыть оверлей до `router.push` экрана практики.
...
4. **Карточка практики** — `Communicator` рендерит общий `modules/practices/ui/PracticeCard.tsx`, переводя серверный `PracticePicked` в `PracticeSummary`. Текст описания в карточке берётся из `**practicePicked.card_blurb`** (если сервер прислал валидированный model-generated blurb или fallback summary) либо из `**reason`** как legacy fallback; новый серверный контракт держит `reason` как текст пузыря о связи практики с днём, а `card_blurb` как текст карточки о самой практике. Для **йоги** `overrideDurationMinutes` / `overrideChakraIndex` **не** передаются (значения каталога); для breath/meditation пользователь может переопределить duration/chakra перед запуском; **`PracticeCard`** клипит `overrideDurationMinutes` к допустимым шагам каталога (см. `assistantSelectableDurations.ts`, тот же контракт, что и сервер в `resolvePracticePublic`). Перед `launchPractice(..., { launchSource: 'assistant' })` компонент вызывает `onPracticePicked` и ждёт Promise, чтобы модальный overlay успел закрыться до навигации на экран практики.

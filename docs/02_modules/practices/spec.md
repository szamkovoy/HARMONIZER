...
- **`CoherenceBreathScreen`** (пропсы см. компонент): мандала + звук + сенсорный режим `fingerCamera | ble | none`; BLE-сценарий может стартовать как с уже remembered chest strap, так и с девайса, только что выбранного в модальном поиске из `PracticeCard`. Для BLE экран больше не показывает отдельный prep/QC-screen, а сразу открывает running-экран практики; если в первые секунды выбранный ремень не выходит в ready-state или отваливается, поверх running-экрана открывается тот же общий `WearablePickerDialog`, чтобы пользователь мог выбрать другой датчик без дублирования UI/логики поиска. Если конкретный `deviceId` ещё не передан, экран всё равно умеет предложить выбор устройства тем же диалогом и по завершении так же пишет **`metrics`** из исхода дыхания (`outcomeToCommunicatorPayload`) в `practice_sessions`. Для `fingerCamera` итоговый экран больше не трактует все метрики как бинарно «есть/нет`: `SignalTrustLevel` из biofeedback-пайплайна даёт три режима. `full_biometrics` показывает coherence/RSA + RMSSD/stress, `guided_limited` сохраняет только pulse-guidance и финальные RMSSD/stress, а `pulse_only` скрывает финальную биометрию и оставляет практику на baseline pulse guidance. При whole-session trust первый минутный отрезок finger-измерения теперь может быть прощён: если после него остаётся ещё как минимум 60 секунд и 30+ metric beats, ранние gap/jitter-события больше не обязаны навсегда понижать итоговый trust. Если деградация произошла только под конец, экран может сохранить `RMSSD/stress` по последнему надёжному хвосту finger-сигнала (не короче 90 с) и явно пометить, что это recovered-tail результат, а не full-session clean capture.
- **`CoherenceBreathScreen`** (пропсы см. компонент): мандала + звук + сенсорный режим `fingerCamera | ble | none`; BLE-сценарий может стартовать как с уже remembered chest strap, так и с девайса, только что выбранного в модальном поиске из `PracticeCard`. Для BLE экран больше не показывает отдельный prep/QC-screen, а сразу открывает running-экран практики; если в первые секунды выбранный ремень не выходит в ready-state или отваливается, поверх running-экрана открывается тот же общий `WearablePickerDialog`, чтобы пользователь мог выбрать другой датчик без дублирования UI/логики поиска. Если конкретный `deviceId` ещё не передан, экран всё равно умеет предложить выбор устройства тем же диалогом и по завершении так же пишет **`metrics`** из исхода дыхания (`outcomeToCommunicatorPayload`) в `practice_sessions`. Для `fingerCamera` итоговый экран больше не трактует все метрики как бинарно «есть/нет»: `SignalTrustLevel` из biofeedback-пайплайна даёт три режима. `full_biometrics` показывает coherence/RSA + RMSSD/stress, `guided_limited` сохраняет только pulse-guidance и финальные RMSSD/stress, а `pulse_only` скрывает финальную биометрию и оставляет практику на baseline pulse guidance. Если деградация произошла только под конец, экран может сохранить `RMSSD/stress` по последнему надёжному хвосту finger-сигнала (не короче 90 с) и отдельно попробовать восстановить `coherence/RSA` по более строгому непрерывному tail-окну 180/150/120 с; оба случая явно помечаются на экране как recovered-tail результаты, а не full-session clean capture.

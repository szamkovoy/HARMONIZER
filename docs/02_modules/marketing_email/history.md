...
- **2026-09-24 (equinox access window):** Волна может на `access_window_hours` включить демо уже вошедшим «Навигаторам» и временный «Мастер» «Наставникам». Не вошедших не трогаем — иначе заполненный `trial_expires_at` блокирует суточное демо первого OTP. Долгий «Мастер» из волны исключается. Снимок отката — `email_campaign_access_grants` (`20260924201000`). `split_half` + `pinned_next_wave_at` для двух равных половин.

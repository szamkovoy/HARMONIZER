...
- **Прогрев волнами:** разовые рассылки не шлют весь сегмент за один клик. `warmup_plan.sizes` по умолчанию `[500,500,1000,1000,2000,2000,3000]` (`repeat_last` → дальше 3000). Получатели сортируются по `greatest(users.last_seen_at, users.getcourse_last_activity_at)`. `audience_cap` (опционально) ограничивает письмо N самыми свежими навсегда. `POST …/campaigns/[id]/send` ставит `sending` и досылает в `after()` + cron `*/5` `/api/cron/email-campaigns` (pg_net 300s). Статусы: `draft` → `sending` → `paused` (можно править текст/сегмент/план) → автоследующая волна в ближайшие **16:00 Europe/Moscow** не раньше чем через 12 ч. `POST …/halt` останавливает очередь; повторный send не трогает `sent|delivered|opened|clicked`. `failed` можно ретраить следующей волной. Lease `send_lease_until` не даёт двум воркерам слать одно письмо. Пока `sending` — UI read-only + «Остановить». Когда eligible кончились — `sent`.

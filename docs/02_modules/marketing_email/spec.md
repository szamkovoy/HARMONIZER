...
- **Окно доступа на волну** (`warmup_plan.access_window_hours`, сейчас 30 у равноденствия): перед постановкой волны в очередь. Уже вошедший «Навигатор» (нет живого демо и оплаченного тарифа) получает демо до `now+hours` — само гаснет обратно в «Навигатор». Живое демо не двигаем. Кто ещё не входил (`app_first_open_at`/`last_seen_at`/`onboarded_at` пусты) не получает дату демо, чтобы первый OTP по-прежнему выдал свои сутки. «Наставник» (`oracle`/`practitioner` с живым сроком) на те же часы становится «Мастер»; снимок срока пишется в `email_campaign_access_grants`, cron возвращает его, если за окно не куплен свой «Мастер». Долгий «Мастер» (срок пуст или дальше 48 ч) в такую волну не входит. Повтор волны не продлевает окно: грант уникален по `(campaign_id, user_id)`.
- **Прогрев волнами:** разовые рассылки не шлют весь сегмент за один клик. `split_half` на старте делит оставшихся пополам (свежие первыми). `pinned_next_wave_at` задаёт точное время следующей волны и снимается после неё. `warmup_plan.sizes` по умолчанию `[500,500,1000,1000,2000,2000,3000]` (`repeat_last` → дальше 3000). `warmup_plan.wave_base_sent` — `sent_count` на старте текущей волны; волна **не** закрывается, пока `queued + sent_this_wave < next_wave_size` (иначе Vercel 300s обрывал набор очереди по одному контакту и индекс прыгал дальше, оставляя день недосланным). Набор волны — upsert пачками по 200. Drain до дедлайна инвока; cron `*/5` досылает queued и, если квота не набрана, докладывает очередь. Resend HTTP 429 — строка остаётся `queued`, инвок выходит, следующий cron продолжит (пауза ~5 мин между пакетами). Получатели сортируются по `greatest(users.last_seen_at, users.getcourse_last_activity_at)`. `audience_cap` (опционально; PATCH) ограничивает письмо N самыми свежими навсегда. `GET …/campaigns/[id]` отдаёт `progress` (`queued`/`accepted`/`failed`/`remaining_estimate`/`next_wave_*`/`halted`); UI poll 5 с пока `sending`. `POST …/send` `{start_wave:true}` (default) → `sending` + drain в `after()` + cron `*/5` `/api/cron/email-campaigns` (pg_net 300s); ответ сразу `{started,status:"sending",…}` без финальных счётчиков волны. Статусы: `draft` → `sending` → `paused` (можно править текст/сегмент/`audience_cap`) → автоследующая волна в ближайшие **16:00 Europe/Moscow** не раньше чем через 12 ч. `POST …/halt` останавливает очередь / снимает автозапуск; повторный send не трогает `sent|delivered|opened|clicked`. `failed` можно ретраить следующей волной. Lease `send_lease_until` + RPC `try_lock_email_campaign_send` не дают двум воркерам слать одно письмо. Пока `sending` — UI read-only + «Остановить». Когда eligible кончились — `sent`.
- **Публичный origin писем:** `EMAIL_PUBLIC_BASE_URL=https://harmonizer.zamkovoi.yoga` (трекинг кликов/открытий, отписка, картинки). `harmonizer-ten.vercel.app` для ссылок в письмах не используется: `*.vercel.app` блокируется в РФ. Уже отправленные письма хранят старый хост в HTML и сами не переписываются. Cron и вебхуки провайдеров остаются на алиасе `vercel.app`.

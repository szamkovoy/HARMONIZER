...
- From display name: RU «Сергей Замковой», иначе «Sergei Zamkovoi» (`marketingSenderName`, как OTP); footer unsubscribe 12.5px; chrome/body **Arial, Helvetica, sans-serif** + явный `font-size:16px` на каждом `<p>` (Яндекс.Почта app не наследует размер с `<td>`/`<div>` и ломает `system-ui` → мелкий текст); `-webkit-text-size-adjust:100%`; блоки: heading/text/image/button (legacy `logo` → `image` при parse)

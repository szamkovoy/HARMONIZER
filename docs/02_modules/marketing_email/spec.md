...
- From display name: RU «Сергей Замковой», иначе «Sergei Zamkovoi» (`marketingSenderName`, как OTP); footer unsubscribe 14px; chrome/body **Arial**; явный `font-size:16px !important` + `<font size>` на абзацах (клиенты вроде Яндекс.Почты/Mail.ru часто игнорируют CSS на `<p>`). Padding только на `<td>` — не на `table width=100%` (иначе письмо шире экрана и приложение уменьшает весь HTML). `-webkit-text-size-adjust:100%`; блоки: heading/text/image/button (legacy `logo` → `image` при parse)

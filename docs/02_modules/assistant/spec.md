...
- Практика выбирается сервером через **`choosePractice`** независимо от старых phase-условий; marker `id="default"` резолвится в когерентное дыхание на 10 минут с чакрой дня. Если `markerId` из маркера не найден в каталоге, пишется лог `[PRACTICE_SELECTOR] marker_id_not_found`, в `meta.practicePicked` добавляется `markerIdResolved: false`. `choosePractice` извлекает предпочтения пользователя (`inferPreferredPracticeKind`, `inferPreferredDurationSec`) по конкатенации **всех** user-сообщений из истории + текущего ввода, а не только по последнему сообщению.
- Если в финальных режимах (`final_recommendation`, `final_recommendation_with_validation_warning`, `forced_final`) маркер `[PRACTICE_PICK]` отсутствует, сервер создаёт синтетический `effectivePracticePick` с `id: "auto-fallback"`: длительность определяется regex-парсингом из текста пользователя, чакра — по `planet_of_the_day` через маппинг планета→чакра; лог `[DIALOG_V3_DIAG] PRACTICE_PICK marker missing`.

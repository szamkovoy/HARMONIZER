...
- Маркеры ответа: **`[READY_FOR_RECOMMENDATION]`**, **`[PRACTICE_PICK: ...]`**, **`[CORRECT_RECOMMENDATION: ...]`**. Сервер вырезает их из видимого текста, но использует для эскалации, выбора практики и коррекции `user_daily_forecasts`.
- Практика выбирается сервером через **`choosePractice`** независимо от старых phase-условий; marker `id="default"` резолвится в когерентное дыхание на 10 минут с чакрой дня.
- В **`messages.meta`** сохраняются `turn_mode`, `model_used` (`standard` / `premium`), `model_id`, `iteration`, `ready_marker_triggered`, `validation`, `practice_picked`, `recommendationCorrected`, `insight_metrics`.

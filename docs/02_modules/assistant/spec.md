...
- Практика выбирается сервером через **`choosePractice`** независимо от старых phase-условий; marker `id="default"` резолвится в когерентное дыхание на 10 минут с чакрой дня. `choosePractice` всегда возвращает `chakraId` (из `planet_of_the_day`) и `preferredDurationMin` (вывод из **всей** истории сообщений пользователя через `inferPreferredDurationSec`, не только из текущего сообщения). Если `markerId` из маркера не найден в каталоге, пишется лог `[PRACTICE_SELECTOR] marker_id_not_found`, в `meta.practicePicked` добавляется `markerIdResolved: false`.
...
| **`dialogArcOrchestrator.ts`** | Короткие инструкции для `opening` / `inquiry` / `forced_final` / `post_recommendation` и функция `decideTurnMode`. |
| **`insightDetection.ts`** | CSI, детектор «инсайта», TTM-стадия по маркерам, валентность, ETV. |
| **`markers.ts`** | Парсинг `[PRACTICE_PICK]` / `[CORRECT_RECOMMENDATION]`, снятие `[READY_FOR_RECOMMENDATION]`, быстрая валидация «есть ли duration и type». |

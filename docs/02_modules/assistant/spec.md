...
| **`markers.ts`** | Парсинг `[PRACTICE_PICK]` / `[CORRECT_RECOMMENDATION]`, снятие `[READY_FOR_RECOMMENDATION]`, валидация и извлечение длительности/типа практики из истории (`validateHistoryHasDurationAndType` → `durationSec`, `practiceKind: PracticeKindInferred`). |

...
code_refs:
  [
    supabase/migrations/20260608203000_dialog_system_v3_v7.sql,
    supabase/migrations/20260608203500_activate_dialog_system_v3_v7.sql,
...
**Сверка с БД:** runtime использует `public.prompts.prompt_key = "dialog_system_v3"` **version 7** (`is_active = true`). Текст §1 ниже — дословное зеркало миграции `supabase/migrations/20260608203000_dialog_system_v3_v7.sql`. Backend-валидация практики и `{{catalog_reconciliation}}` опираются на более узкие диапазоны в `markers.ts` (**1–5 / 5–20 / 20–70** минут) — см. §3.1; при расхождении с формулировками v7 источник истины для эскалации и `practice_picked` — backend. Sphere-only `spheres="..."` для `[PLANNED_EVENT]` и planning-финал задаются server-side guardrails в `route.ts` (§3.1), а не текстом v7 в §1.
...
| `{{chakra_label}}`               | Название активной чакры на русском      | `chakraLabelRu(chakraMeta.chakra_number)` из `@/modules/chakra/labels`                                                                                                                                                |
| `{{chakra_label_accusative}}`    | Название активной чакры в винительном падеже | `chakraLabelAccusativeRu(chakraMeta.chakra_number)` в `route.ts`                                                                                                                                             |
...
| Компонент | Поведение |
| --------- | --------- |
| `catalogDurationRangeForKind` (`markers.ts`) | Backend-каталог: медитация **1–5**, дыхание **5–20**, асаны **20–70** минут. Используется в `validateHistoryHasDurationAndType`, `buildCatalogReconciliationInstruction`, `resolvePracticePublic` (клип к `assistantSelectableDurations`). Текст v7 в §1 может формулировать дыхание как **6–20** и не называть потолок асан — для runtime authoritative backend. |
...
| Planning guardrails в `buildDialogSystemInstruction` | Дополнительный runtime-блок для `planning`: только действия/события; без уточнения состояний/подтекста/«первая или вторая половина дня»; без технических деталей эпизода; practice-replies не превращаются в `[PLANNED_EVENT]`; каждый `[PLANNED_EVENT]` несёт `recommendation`, `display_order` и только sphere-only `spheres`, planning-финал — через `[CORRECT_RECOMMENDATION]`; при наличии `context.planningSphereLens` добавляется 7-дневный сигнал по слабо представленным сферам. |
| `planningAlreadyFinalized` / `buildFinalInstructionText` | Если предыдущий assistant-turn уже финализировал planning (`[CORRECT_RECOMMENDATION]` или явный practice-вопрос), premium `final_recommendation*` получает lock: не повторять day-focus и per-event советы, только practice-specific мостик после `[PRACTICE_PICK]`. |
| `assessDueEventSummary` / synthetic summary advance | При `daySummaryRequested` route может автоматически закрыть текущее due-событие без `[SUMMARIZE_EVENT]`, если ответ достаточен или второй ответ остался расплывчатым; при `occurred_without_state` на первом ответе `[SUMMARIZE_EVENT]` отбрасывается до follow-up. |

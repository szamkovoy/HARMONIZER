...
Первый ход ассистента (`iteration === 1`), в истории ещё нет сообщения ассистента с `practice_picked` / `practicePicked` в `meta`, в сессии **ровно одна** пользовательская реплика до ответа ассистента (текущий `userMessage`, ещё не записанный в БД на момент решения, при пустой истории пользователя; либо единственная уже сохранённая реплика пользователя при отсутствии нового текста в запросе), и по тексту этой реплики `validateHistoryHasDurationAndType` на массиве из одного сообщения вернул `confident: true`. Модель **premium**; `streamGeminiText` со стримингом в UI **без** предварительного standard-вызова и **без** проверки маркера `[READY_FOR_RECOMMENDATION]`.

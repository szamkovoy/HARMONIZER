...
**Сверка с БД:** при подготовке этого файла таблица `public.prompts` не опрашивалась. Текст ниже скопирован **дословно** из поля `template` в миграции `supabase/migrations/20260519030000_dialog_system_v3_v5_activate.sql` (активный `dialog_system_v3` **version 5**; двухфазный контур summarizing/planning, matrix markers, target chakra). Подготовительная миграция без активации: `20260516150000_dialog_system_v3_v5.sql`. Если активная строка в БД отличается — синхронизируйте вручную и обновите справочник после сверки.
...
| `{{practice_refusal_check}}`     | Одноходовый флаг для `inquiry`         | `decideTurnMode(...)` в `dialogArcOrchestrator.ts`: когда `unresolvedPracticePromptCount >= getPracticeRefusalThreshold()` (env `PRACTICE_REFUSAL_THRESHOLD`, дефолт **1**) подставляется `PRACTICE_REFUSAL_CHECK_INSTRUCTION`; иначе пустая строка |

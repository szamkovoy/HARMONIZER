...
`userDeclinedPracticeInHistory(...)` в `markers.ts` вернул `true` по репликам пользователя (включая pending); модель **standard**; `generateGeminiText`. Не входит в объект `ORCHESTRATOR_INSTRUCTIONS` — отдельная константа `PRACTICE_DECLINED_SESSION_INSTRUCTION`. Режим терминальный: один ответ и закрытие диалога.

...
**Сверка с БД:** при подготовке этого файла таблица `public.prompts` не опрашивалась. Текст ниже скопирован **дословно** из поля `template` в миграции `supabase/migrations/20260515030000_dialog_system_v3_pack_b.sql` (активный `dialog_system_v3` **version 4**; пакет `B`: антизатягивание после готовности, явный отказ от практики, запрет описывать технику в тексте). Предыдущая миграция: `20260513224500_dialog_system_v3_polish_pack_a_fix.sql` (v3). Если активная строка в БД отличается — синхронизируйте вручную и обновите справочник после сверки.
...
| `{{practice_refusal_check}}`     | Одноходовый флаг для `inquiry`         | `decideTurnMode(...)` в `dialogArcOrchestrator.ts`: после двух подряд `opening` / `inquiry` без уверенного ответа о длительности и типе подставляется текст прямого вопроса о том, нужна ли практика вообще; иначе пустая строка |

...
- **Wearable path:** `useWearableScanner` ищет BLE-устройства по Heart Rate Service / имени, `BleHeartRateSource` подключается через `@sfourdrinier/react-native-ble-plx`, парсит characteristic `0x2A37`, конвертирует RR (`1/1024 s`) в beat timestamps и отдаёт их в тот же pipeline. Сброс RR-timeline в `BleHeartRateSource` завязан на **паузу между BLE-пакетами** (`lastRrAtMs`), а не на разницу wall clock ↔ beat timestamp — иначе chest-strap даёт редкие «прыжки» RR 5–10 с и пустую тахограмму. Общий `WearablePickerDialog` поднимает этот сканер (окно **12 с**, затем «не найден») и переиспользуется в двух местах: до старта практики из `PracticeCard` и уже поверх running-экрана, если выбранный BLE-датчик не выходит в ready-state или теряет связь. Для trusted Polar-профилей (`Polar H10`, `Polar H9`) UI помечает источник как `polarEnhanced`, но downstream-метрики остаются общими.

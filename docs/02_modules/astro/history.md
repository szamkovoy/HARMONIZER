...
- **2026-07-20:** `POST /api/astro/natal` после записи натала дополнительно удаляет `scenario_cache` где `scenario_id='morning_recommendation'` для пользователя (раньше чистили только `user_daily_forecasts` ≥ сегодня). Иначе числовой прогноз пересчитывался по новому наталу, а LLM-слоган/рекомендация оставались в кэше, keyed только `user+date+locale` — на Home диаграмма и текст расходились (напр. Венера vs текст про Меркурий). См. `daily_forecast/history.md` 2026-07-20.

- **2026-07-18 (timezone audit — historical offset handling):** Подтверждено, что расчёт натальной карты уже корректно учитывает исторические таймзоны: `localChartDateTime` (`modules/astro-core/ephemeris.ts`) строит локальный момент через `luxon.DateTime.fromISO(date+time, { zone: location.timezone })` и делает `.toUTC()` — Luxon разрешает смещение для исторической даты по IANA tz-базе (DST, смена правил, политические изменения). IANA-зона берётся из Open-Meteo Geocoding (`GeoPlace.timezone`) и прокидывается через мастер → `natalProfileClient` → `POST /api/astro/natal`. Отдельная «UTC-колонка» не хранится — UTC воспроизводится из канонических входов (`birth_date`/`birth_time`/`birth_place.timezone`/`tz`) при каждом расчёте. Зафиксировано в `spec.md` §3 (новый пункт) и §5 (граничные случаи: смена IANA-зоны, DST fall-back, полнота tz-данных платформы). Кода не меняли.

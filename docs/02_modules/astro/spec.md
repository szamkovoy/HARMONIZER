...
- `services/natalProfileClient.ts`: `createNatalProfile(birthData, signal?)` — `POST` на `getAstroNatalUrl()` (Vercel `_legacy_web/app/api/astro/natal`) с телом `{ birthData }` и JWT Supabase; ответ содержит `profile: NatalProfile` и опционально `natalChart` (строка БД). Клиентский save-path защищён таймаутом **30s**, чтобы modal «Сохраняю...» не зависал бесконечно при подвисшем fetch/сервере.
...
- `fetchActiveNatalProfileCached(userId, options?)` — mobile/web helper для UI карты на Home: при наличии **непустого** локального кэша (`expo-secure-store` / `localStorage`, ключ `harmonizer.natalProfile.v1.{userId}`) сразу возвращает его и в фоне обновляет из сети; опциональные `options.onBackgroundRefresh` и `options.expectedBirthFingerprint`. Если ожидаемый fingerprint из текущих `users.birth_*` не совпал с локальной записью, тёплый кэш **игнорируется** и перечитывается сетью, чтобы после смены даты рождения/reopen не показать устаревшую карту. Запись кэша с `profile: null` при чтении сбрасывается (не short-circuit), сетевые сбои **не** персистятся как «нет карты». При отсутствии валидного кэша ждёт сетевой fetch. Запуск персонального прогноза на Home **не** зависит от успеха этого helper — gate по `users.birth_date` (см. `daily_forecast`).
...
- **Локальное время → UTC (исторические таймзоны):** эфемеридам нужен единый момент времени. Преобразование делает `localChartDateTime` в `modules/astro-core/ephemeris.ts` (и зеркалах): `DateTime.fromISO(\`${date}T${time}\`, { zone: location.timezone })` → `.toUTC().toJSDate()`. Зона `location.timezone` — IANA-идентификатор из Open-Meteo Geocoding (см. `modules/onboarding/geoSearchClient.ts` `GeoPlace.timezone`), который прокидывается клиентом из мастера (`app/onboarding.tsx` → `services/natalProfileClient.ts` → `POST /api/astro/natal`). **Ключевое:** IANA-имя зоны стабильно во времени (например `Europe/Moscow`), а смещение для исторической даты (DST, смена правил, политические изменения) разрешает **Luxon** по базе IANA tz — поэтому для рождения 40 лет назад берётся смещение, действовавшее тогда, а не текущее. `timeMode: "unknown"` фиксирует полдень локальной зоны. Хранение: в `users` пишутся канонические входы — `birth_date`, `birth_time`, `birth_place` (jsonb: name/lat/lon/timezone) и `tz` (IANA); отдельная колонка «вычисленный UTC» **не** хранится — UTC-момент детерминированно воспроизводится из локального времени + IANA-зоны при каждом расчёте, что устойчиво к корректировкам tz-базы и исключает дрейф.
...
- **Граничные случаи таймзоны:** (а) Open-Meteo возвращает **текущий** IANA-идентификатор города; для подавляющего большинства мест имя зоны не меняется десятилетиями, а исторические смещения разрешает Luxon по tz-базе — но для редких регионов, сменивших IANA-зону, ретроспективное смещение может быть чуть неточным. (б) При рождении в час «перевода часов назад» (DST fall-back) локальное wall-clock время неоднозначно (повторяется дважды); Luxon по умолчанию берёт первое вхождение — погрешность ≤1 ч только для этого часа. (в) Точность зависит от полноты tz-данных платформы: iOS/Android (ICU) и Node на Vercel содержат исторические tz-данные; в средах с урезанной tz-базой потребовалось бы тащить данные отдельно (здесь неактуально).

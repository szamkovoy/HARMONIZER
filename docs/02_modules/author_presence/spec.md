...
- UI `/admin/posts` («Видео»): infinite scroll (по 20); список с `pickAdminPostDisplay` (title+cover **одного** языка RU→EN→…); статусы **Черновик** / **Планируется** (`is_published` + `published_at` в будущем) / **Опубликовано**; ссылка `?tab=<locale>` открывает редактор на этой вкладке; **создание** после Save → список, **редактирование** остаётся в форме; рядом с чекбоксом «Опубликовать» — дата публикации и **длительность** (ЧЧ/ММ/СС → `duration_seconds`, общая для всех языков; бейдж на обложке как YouTube); без галочки кнопка «Запланировать» (черновик); с галочкой + будущая дата → в приложении скрыто до даты (`get_posts_feed` / RLS: `published_at <= now()`); обложка с compress + `object-contain`; «Перевести» fill-missing; «Удалить перевод» / «Удалить обложку»; 🌐; ФИО комментария → `/admin/users/[id]`.

...
- **`StoriesRing`** — интерактивное кольцо встроено в левый аватар `HomeHeader` (`app/(tabs)/index.tsx`), отдельного блока под шапкой больше нет. На фокусе главной грузит **весь активный feed** (`refreshStoryFeedInBackground`) и анимированно раскрывает сегменты. Фоновый poll (`StorySessionBootstrap`) **адаптивный**: `storyFeedPollDelay` — 20 с в первые 3 мин после запуска/возврата в foreground, затем 90 с; таймер живёт только при `AppState === "active"` (в background не опрашивает), возврат в foreground = немедленный refresh + сброс окна. Feed — **stale-while-revalidate**: UI сразу рисует warm-cache (`peekStoryFeedForUi`); при ошибке RPC/сети предыдущий feed **не** затирается пустым массивом (раньше из‑за `return []` на ошибке кольцо на секунды/дольше сменялось бренд-аватаром). Cold start без cache: quiet placeholder + один короткий retry, бренд-аватар только после settled empty. Успешный пустой ответ по-прежнему очищает кольцо. Новые story ID — soft hold до 8s (ранний выход после 1.5s, если poster JPEG уже в кэше). Для ближайших video-story клиент дополнительно прогревает **локальный `file://` mp4-cache** через `expo-file-system`: `ensureStoryReadyToOpen` и фоновые prefetch-и докачивают ролик в `Paths.cache/stories-video/*` ещё до открытия viewer, поэтому первый показ нового видео больше не зависит от cold network start. HTTP Range warm mp4 не используется (ломает shared URL cache → зелёные кадры). В центре — session-cached `thumbnail_url`; без сторис — бренд-аватар.
...
- **`LatestPostBanner`** — полная `VideoCard` **под** «Окнами возможностей»; только самое свежее видео/запись вебинара **с title для активной UI-локали** (exact; RPC `p_locale` + `postAvailableInLocale`), которое пользователь ещё не открывал. `fetchLatestUnviewedPostForLocale` мемоизирован на 60 s по `(locale, userId)` (баннер размонтируется при переходе Home loading → ready и перезапускается `useFocusEffect` — без memo это 2×(`get_posts_feed` + `user_post_views`) на старте); `markPostViewed` сбрасывает memo.
...
- UI `/admin/posts` («Видео»): infinite scroll (по 20); список с `pickAdminPostDisplay` (title+cover **одного** языка RU→EN→…); статусы **Черновик** / **Планируется** / **Опубликовано**; **создание и редактирование** после Save → список; дата + длительность ЧЧ/ММ/СС; 🌐 только при реальном `title_i18n`/`body_i18n`/`cover_url_i18n` (не по пустому `translations_updated_at`); «Перевести» fill-missing с активной вкладки (RU→все; EN→все кроме RU); ФИО комментария → `/admin/users/[id]`.
- Клиент: `VideoCard` / `PostScreen` показывают `duration_seconds` бейджем в правом нижнем углу обложки (`formatVideoDuration`).
...
- **`posts`** (`20260708130000` + `20260709100000` + `20260728212801`): `title`, `body` (plain text: переносы + URL), `cover_url`, `is_published`, `published_at`, `duration_seconds` (целое, locale-agnostic; null = не задано), `created_by`; плюс i18n-поля: `title_i18n` / `body_i18n` / `cover_url_i18n` (jsonb, DEFAULT `{}`), `translations_updated_at` (timestamptz). RLS: public read опубликованных (`published_at <= now()`), admin write. Индекс `published_at desc where is_published`.

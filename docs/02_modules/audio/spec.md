...
- **Фоновое воспроизведение (Android 14+).** `expo-audio` с config-плагином `enableBackgroundPlayback: true` добавляет в `AndroidManifest` foreground service `AudioControlsService` (тип `mediaPlayback`) + permissions `FOREGROUND_SERVICE` / `FOREGROUND_SERVICE_MEDIA_PLAYBACK`. Для длительного фонового звука (часы) на Android **обязателен** вызов `player.setActiveForLockScreen(true, metadata)` — без него ОС останавливает аудио примерно через 3 минуты в Doze (platform limitation, см. docs expo-audio). `MandalaSoundProvider` при `staysActiveInBackground + lockScreen` резолвит обложку в локальный `file://` URI (`resolveLocalArtworkUri` через `expo-asset`) и передаёт в движок; движок bindит lock-screen на lead-плеере (drone для neuro-sync, единственный буфер для ambient — привязка один раз при старте, без ре-биндинга). Метаданные карточки: `title` = `lockScreen.title` (локализованное название практики, передаёт `CalmPracticeScreen` из `catalogStrings` — следует за profile-локалью), `artist` = `getAppDisplayName(locale)` из `@/modules/i18n` (локализованное имя приложения: RU «Гармонизатор», EN «Harmonizer» … — runtime-зеркало build-time `APP_NAMES` из `plugins/appLocalesData.js`), `artworkUrl` = локальный URI обложки. Локаль берётся через `useAppLocale()`; `locale` входит в dep-массив audio-lifecycle effect-а, поэтому карточка перебиндится при смене языка профиля. `interruptionMode: "doNotMix"` — обязательно для `setActiveForLockScreen` и означает, что медитация становится единственным аудио (чужое приложение ставится на паузу). Для не-фоновых практик (breath/Flash) используется `"duckOthers"` — bed слышно, но чужое аудио не останавливается полностью.
...
- **Ambient loop — single-buffer native loop.** `AmbientLoopEngine` использует один буфер с `player.loop = true` (нативный ExoPlayer `REPEAT_MODE_ONE`). `MediaSession` привязывается к lock-screen ОДИН раз при старте (`setActiveForLockScreen`) и больше не ребиндится — foreground service, audio focus и lock-screen виджет стабильны всю практику (часы). Раннее dual-buffer A/B-handoff с кроссфейдом (маскировка микро-паузы `REPEAT_MODE_ONE` на стыке) откатан: на Android `expo-audio` привязывает `MediaSession` к конкретному `AudioPlayer`, поэтому любой A/B-сwap требовал ребинда сессии mid-playback — это фундаментально деструктивно (ducking исходящего буфера / потеря фокуса / пустой виджет), и две итерации (rebind-after, rebind-before) обе давали регрессии. Возможный микро-провал на стыке лупа смягчён pre-baked gapless-ассетами (`build-ambient-loops.mjs`). Прогресс-бар lock-screen показывает длительность лупа (короткую) и перескакивает на каждом цикле — косметика; сделать бар на всю длительность практики (8 ч) можно только нативным патчем (`ForwardingPlayer`-обёртки, переопределяющей `getDuration()`/`getCurrentPosition()`) — отложено как требующее device-тестирования. Приоритет продукта №1 (часы фонового звука) важнее косметики стыка.
...
- Внешние входы провайдера: `practiceKind: "breath" | "meditation"`, `durationMs`, `chakra`, `isActive`, `plannedCycle`, `cycleStartMs`, `biofeedbackEnabled`, **`soundBed`** (`neuro-sync` | `creek` | `waves` | `rain` | `forest_birds` | `wind` | `fireplace` | `water_splash` | `cat_purr`), **`staysActiveInBackground`** (фоновое воспроизведение для «Спокойствие»), **`lockScreen`** (`{ title, artwork }` — карта media-notification / lock-screen: локализованное название практики + `require()`'d обложка; bindится только вместе с `staysActiveInBackground`). `artist` карточки (имя приложения) провайдер не принимает снаружи, а вычисляет сам из активной локали через `getAppDisplayName(locale)` (`@/modules/i18n`) — следует за profile-локалью, как и `title`.

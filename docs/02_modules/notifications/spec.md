...
- **Android exact alarms (окна возможностей):** `expo-notifications` `ExpoSchedulingDelegate` вызывает `setExactAndAllowWhileIdle` только при `AlarmManager.canScheduleExactAlarms()`; иначе inexact. Манифест: `SCHEDULE_EXACT_ALARM` (`app.json`). Runtime: `ensureAndroidExactAlarmsForOpportunityReminders` через native bridge `harmonizer-android-exact-alarms` + `expo-intent-launcher` `REQUEST_SCHEDULE_EXACT_ALARM`. Whitelist батареи / OEM autostart не используем — exact alarm достаточен для DATE-напоминаний.

...
| `opportunity_bell` | Сохранение напоминания колокольчиком | Явный жест: сначала закрыть модалку «Уведомить», затем системный `requestPermissionsAsync` (без cooldown). Кастомный Alert — только если ОС не выдала `granted` (отказ или `canAskAgain=false`); в Alert — «Открыть настройки». Имя приложения в тексте — `common.appName` через `{appName}`. **Android 12+:** после notification grant — `ensureAndroidOpportunityReminderPrerequisites` (`services/androidExactAlarms.ts`): (1) exact alarm / «Будильники и напоминания»; (2) API 23+ whitelist battery optimization (`REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`); (3) на Xiaomi/Huawei/Oppo/Vivo/OnePlus/Meizu — экран автозапуска/фона (best-effort intent). Напоминание не ставится, пока проверяемые шаги не пройдены. |
...
- **Android exact alarms (окна возможностей):** `expo-notifications` `ExpoSchedulingDelegate` вызывает `setExactAndAllowWhileIdle` только при `AlarmManager.canScheduleExactAlarms()`; иначе inexact. Манифест: `SCHEDULE_EXACT_ALARM` + `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` (`app.json`). Runtime: `ensureAndroidOpportunityReminderPrerequisites` — exact alarm (`harmonizer-android-exact-alarms`), battery whitelist (API 23+), OEM autostart/фон (Xiaomi/Huawei/Oppo/Vivo/OnePlus/Meizu, intent best-effort).

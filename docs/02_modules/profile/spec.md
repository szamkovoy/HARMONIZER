...
- **Отчёты HARMONIZER v2** — **четыре независимые карточки** (`ProfileReportCard`) в порядке: «Статистика практик», «Практики по чакрам», «Матрица состояний», «Толщина линии жизни». Селектор периода — **только** у первых двух (под заголовком). «Матрица» и «Толщина» — за всю историю, без селектора. Активный день = `planned_events.status = 'summarized'` по `planned_local_date`; порог готовности матрицы/тренда — **≥ 5** таких дней. Локализация — `modules/profile/i18n/profile.ts`. Два backend endpoint-а:
  - `GET /api/profile/life-matrix` — без `days`; активные дни из `planned_events`, агрегат матрицы за всю историю, `calendarTrend` — блоки по 5 активных дней (`range_metric` на суммированной матрице блока, ось X — календарная дата последнего дня блока);
  ...
  Клиент: `modules/profile/ui/ProfileReports.tsx` (`PracticeByChakraReportCard`, `LifeMatrixReportCard`, `RangeTrendReportCard`, `useLifeMatrixReport`), `ProfileEmptyState`, `RangeTrendChart`. Transport: `services/profileReports.ts` (Bearer JWT). Gate — ключ `stats`.

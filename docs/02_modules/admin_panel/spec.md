...
- `POST /api/admin/payments/refund` — `{ contractId, mode: lavatop_mark|yookassa_api|yookassa_mark }` (один контракт из списка платежей и из карточки пользователя). Успех → статус `refunded` + исключение settlement из net-статистики + снятие/пересчёт тарифа подписки (лучший из оставшихся контрактов/грантов, иначе free). `yookassa_api`: preflight GET payment → refund; БД только при `status=succeeded`; иначе 502 + понятная RU-ошибка (чужой shopId / нет средств) + UI «Сделать возврат вручную».

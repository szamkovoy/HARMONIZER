...
- `GET /api/admin/users?…` — фильтры (до 100 строк): `access` (trial/navigator/oracle/master/**not_in_harmonizer**), activity days (`last_seen_within_days` / `last_seen_older_than_days`), addon, locale/geo/marketing, onboarded/created ranges; **sort** (`created_at`|`onboarded_at`|`tier_end`|`last_seen`|`last_payment`|`access`|`locale`) + `order`. RPC `admin_search_users` (миграция `20260804140000`). UI: один фильтр «Доступ сейчас» (без сырого tier); период тарифа рядом с бейджем; во 2-й строке email + «заходил». Доступ: **платный > демо**. После списка — batch-enrich `auto_renew_cancelled` (платный доступ есть, нет `active` subscription-контракта, есть `cancelled` с `cancelled_at`); UI: текст «Отменил автоплатеж» сразу после `AccessNowBadge` (не внутри пузыря). `GET /api/admin/users/[id]` — то же поле на `user`; subscription подгружается как latest `active` **или** `cancelled` (`product_kind=subscription`); при отмене — строка «Автопродление: отменено · доступ до …», кнопка «Отменить оплату» только при `status=active`. Блок «Тариф» (RU + период) + «изменить» → `PATCH {action:"set_access", access, starts_at, ends_at}` (колонка `membership_started_at`); `email_history` со статусом доставки; уведомления — прочитано/нет (`read_at`). `DELETE …/subscription` — отмена оплаты. `POST …/messaging` — `launch_chain` / `cancel_chain` / `send_campaign`. `GET /api/admin/users/stats?days=7|30|90|all` — всего = `onboarded_at IS NOT NULL`; «Распределение по тарифам сейчас» (Демо→…→Мастер); «Покупали допов» = unique users с active `payment_contracts` webinar/book за период.

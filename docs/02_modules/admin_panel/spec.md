...
- `GET /api/admin/users?…` — те же фильтры (до 100 строк); UI `/admin/users` — бейдж списка = **доступ сейчас** (`trial_expires_at > now()` → «Демо», иначе navigator/oracle/master по `membership_*`); фильтр «Демо»/`access=trial` — тот же сегмент; «Тариф в БД (сырой)» = `membership_tier` (free→подпись «Навигатор», не демо). VISIBLE_PRODUCT_TIERS без practitioner/webinar/book. `GET /api/admin/users/[id]` — блоки **Общее** / **Гармонизатор**: бейдж доступа сейчас + поля `trial_expires_at` / сырой `membership_tier`; `contact.marketing_status`, `subscription` (active contract), `active_enrollments` (status=active), `email_history`/`notifications` ≤10 + totals; история писем: рассылка → имя кампании; цепочка → имя цепочки + имя письма. City repair; местонахождение — Google Maps search URL. `DELETE …/subscription` — отмена оплаты. `POST …/messaging` — `launch_chain` / `cancel_chain` / `send_campaign`. `GET /api/admin/users/stats?days=7|30|90|all` — всего = `onboarded_at IS NOT NULL`; «Распределение по тарифам сейчас» (Демо→…→Мастер); «Покупали допов» = unique users с active `payment_contracts` webinar/book за период; series / `by_country` / `by_locale` по `onboarded_at` в периоде (нули по языкам не отдаются); UI — единая сетка полос, заголовки «Страны/Языки за …».

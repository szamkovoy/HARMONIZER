...
- `POST /api/admin/users` — ручное добавление в маркетинговую базу без OTP (`createUser` + `email_confirm`, `crm_imported_at`, сегмент email_only). Body: `email` (обяз.) + опц. имя/фамилия/телефон/страна/город/`birth_date`. Пустые и UI-placeholder строки (`Имя`, `Фамилия`, `Телефон`, `Город`…) **не** пишутся в БД — только реально заполненные поля. До create не вызываем `generateLink` (иначе ложный «уже есть» и пустой CRM-профиль). Незавершённый auth-only ряд дописывается в CRM и возвращает **201**. Реальный дубликат — **409** `{ code:"exists", userId, email }`. UI после успеха — «Пользователь добавлен» + ссылка на карточку / «Закрыть». Список: для email_only — `getcourse_last_activity_at` (если есть); страна/город из полей строки (без доп. запросов); в карточке списка — имя + фамилия, если фамилия есть.

...
- `GET /api/admin/users?…` — фильтры (до 100 строк): `access` (trial/navigator/oracle/master/**not_in_harmonizer**/**email_only**), activity days (`last_seen_within_days` / `last_seen_older_than_days`), addon, locale/geo/marketing, onboarded/created ranges; **sort** (`created_at`|`onboarded_at`|`tier_end`|`last_seen`|`last_payment`|`access`|`locale`) + `order`. RPC `admin_search_users` (миграции `20260804140000` + `20260808180000`). UI списка: чипы сегментов как в рассылке (Все / Демо / Не в гармонизаторе / Только рассылки / Навигатор / Наставник / Мастер); даты «Регистрация в системе» (`created_at`) и «Регистрация в Гармонизаторе» (`onboarded_at`). Доступ: **платный > демо**. После списка — batch-enrich `auto_renew_cancelled` + `current_period_end` / `subscription_status`; период тарифа как в карточке (`period_end − 30d` … `current_period_end`, без grace); при `active` — «след. платёж» = `current_period_end`. UI: «Отменил автоплатеж» сразу после `AccessNowBadge`.
...
- RPC `admin_dashboard_pulse` (v3 + fix `20260809160000`): KPI — пользователи (всего/24ч/7д), когортная конверсия (рег / купили oracle / купили master за период), распределение тарифов «сейчас»; series с итогами; воронки подписки oracle/master (мес. 1–7): глубина = невозвращённые `payment_settlements` на **первом** оплаченном subscription-контракте (не re-checkout); месяц k≥2 в расчёте только после `(k−1)×30d`; UI: число и **% / ширина от 1-го месяца** (монотонно не шире предыдущего шага); пока `eligible=0` — «ещё рано», не 0%; ручные гранты не входят. LLM 24ч + серия токенов; geo по новым за период. KPI «Выручка» — net Lava.top+ЮКасса за период.

...
- `POST /api/admin/payments/refund` — `{ contractId, mode: lavatop_mark|yookassa_api|yookassa_mark }`. Успех → статус `refunded` + исключение settlement из net-статистики + снятие/пересчёт тарифа подписки. `yookassa_api` меняет БД только при ответе ЮKassa `status=succeeded`; иначе 502 + UI «Сделать возврат вручную».

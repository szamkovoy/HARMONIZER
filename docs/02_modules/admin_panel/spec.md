...
**Рассылки / marketing_email:** nav «Рассылки» → `/admin/email`; цепочки `/admin/email/automations` + редактор `[id]`; deliverability `/admin/email/deliverability`; карточка пользователя — история писем/пушей, send, launch chain, `skip_email_automations`. Контракт — `02_modules/marketing_email/spec.md`.
...
- `GET /api/admin/users?q=&tier=` — поиск по имени/email + фильтр тарифа (до 100 строк). `GET /api/admin/users/[id]` — карточка: профиль, email, платежи, активность, облегчённые `email_history` / `notifications` (ссылки на `/admin/email/*`, `/admin/notifications/[id]`), `skip_email_automations`; stale paid → `recomputeUserMembershipFromPayments`. `PATCH /api/admin/users/[id]` `{tier,…}` или `{skip_email_automations}`. `POST /api/admin/users/[id]/messaging` — `launch_chain` / `send_campaign` (UI подтверждает перед отправкой). Пикер кампаний: `GET /api/admin/email/campaigns?limit=10`.

...
- `GET /api/admin/users?…` — те же фильтры (до 100 строк); UI `/admin/users` — VISIBLE_PRODUCT_TIERS без practitioner/webinar/book. `GET /api/admin/users/[id]` — блоки **Общее** / **Гармонизатор**: `contact.marketing_status`, `subscription` (active contract), `email_history`/`notifications` ≤10 + totals; city repair (district/village → town via Nominatim zoom=10); местонахождение — ссылка на Google Maps search URL по `lat,lon` (без Maps API на загрузке карточки). `DELETE …/subscription` — отмена оплаты. `PATCH` / `POST …/messaging` как ранее. `GET /api/admin/users/stats?days=7|30|90|all` — всего = `onboarded_at IS NOT NULL`; «Распределение по тарифам сейчас» (Демо→…→Мастер); «Покупали допов» = unique users с active `payment_contracts` webinar/book за период; series по `onboarded_at`.

...
DIALOG_SCAFFOLD_SOURCE="_legacy_web/data/dialog_scaffold/ru.json"
TYPED_MANIFEST="modules/i18n/typed/manifest.json"
TYPED_SOURCE="modules/chakra/i18n/chakraTypedSource.json"
CHANGED="$(git diff --name-only "$PUSH_RANGE" -- "$SOURCE" "$DIALOG_SCAFFOLD_SOURCE" "$TYPED_MANIFEST" "$TYPED_SOURCE" modules/profile/i18n/ modules/home/i18n/ modules/day/i18n/ modules/practices/i18n/ modules/communicator/i18n/ modules/breath/i18n/ modules/mandala/i18n/ modules/ui/i18n/ || true)"
if [ -z "$CHANGED" ]; then
...
CATALOG_CHANGED="$(git diff --name-only -- modules/i18n/catalog/ modules/i18n/typed/ _legacy_web/data/dialog_scaffold/ || true)"
if [ -z "$CATALOG_CHANGED" ]; then
  echo "[i18n-sync] No catalog files changed. Done."
  exit 0
fi

git add modules/i18n/catalog/ modules/i18n/typed/ modules/i18n/typed/generated-overlays.ts _legacy_web/data/dialog_scaffold/
